...
- **Accounted-for is not delivered — the conservation gate (#1064).** The rest of this section proves a module was ACCOUNTED FOR. It does not prove that module's findings REACHED THE DELIVERABLE, and a whole recurring defect class (#1040/#1045/#1050/#1043/#1061/#1062) lives in that gap: the detector works, the finding is dropped at a producer→consumer seam, the ledger reads clean and the run exits 0. `pnpm exec tsx src/cli/validate-conservation.ts` closes it — it runs the real ten-module orchestrator against `targets/calibration`, assembles the real deliverable, and asserts per module that its PLANTED finding was both produced by that module's own probe and present in the assembled document. **Zero findings for a planted defect is a FAIL** (zero is legitimate in the field, never on the fixture). The produced/delivered split is load-bearing: M9 captures `detect-static` unfiltered, so a deliverable-only check still sees M7's rows after M7's capture breaks entirely — that is how #1062 hid. M2 has no offline plant and is recorded in `UNEXERCISED` with a #1033 falsifier (`--require M2`), never dropped; a module in neither list throws at load. The CLI needs the mechanical binaries, so — like `dry-run-drift`/`corpus-drift` — it is NOT part of `pnpm verify`; its logic and wiring are, via `src/audit-conservation.test.ts` and `src/cli/validate-conservation.test.ts`. Record: `docs/design/conservation-of-findings.md`.
- **Known gaps in what the guard actually proves** — the running record of what has been closed and what is still open (last updated 2026-07-26) lives in `docs/design/coverage-guard-status.md`. **Still open:** each out-of-orchestrator pass emitting its artifact routinely end-to-end; a workflow running the conservation gate on a schedule (operator action — `.github/workflows/` is supervised).
