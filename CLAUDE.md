...
- **Known gaps in what the guard actually proves (updated 2026-07-16 — the 2026-07-15 gaps below all CLOSED this sweep):** the exit-0-as-evidence blindness is closed (#350) — M5/M8/M9 now derive status from the tool's machine-readable output, and a scan of zero files records `partial`/`requires-live-run`, never `ran`. M6's never-run alarm can no longer be cleared by an unread review packet (#351) — it reports `partial` until a reviewed verdict is recorded. The coverage ledger now HAS a path into the client report (#349): `run-audit --findings-out` assembles the derived ledger into the engagement findings.json and `report-template` renders per-module coverage, so a module that never ran shows as a "Not run" row with its reason rather than silence. **Still open:** findings from the non-`Finding[]` modules M1/M2/M6/M10 are not yet auto-collected into the deliverable — they surface as coverage rows, not findings (M3/M8 capture + explicit non-collection landed in #420; M1/M10/M8-mutant/M7-advisor capture is tracked in #434/#435/#436), and the orchestrator's derive-`ran`-from-artifact upgrade for M1/M2/M3/M6 is #416.
