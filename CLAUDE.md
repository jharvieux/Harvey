...
- **Prisma/Postgres apps are now supported (not just Supabase), detection-gated (#756 core landed 2026-07-23):** `detectOrm` (#757) routes a Prisma app to the app-layer M1 tier (Supabase RLS detectors gate off as N/A-by-architecture) plus a Prisma tenant-scope/BOLA detector (#760); M10 classifies `schema.prisma` (#758); M2 stands up plain Postgres + `prisma migrate` + app-route probing with the PostgREST/RLS matrix recorded not-applicable (#759, proven live against boxyhq). Still open on the epic: M7 Prisma perf (#761, deferrable), authenticated cross-tenant app-route probing (#787), and docs/website/SEO (#762–#764).
