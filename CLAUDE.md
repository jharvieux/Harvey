...
**Run modes are ONE orchestrator (`pnpm exec tsx src/cli/run-audit.ts <t>`) gated by env flags, not three tools.** Every mode runs all ten modules; a tier not in scope records `requires-live-run`/`partial` with a reason, never a skip (the flags declare which environments the engagement HAS, not which modules to run).
