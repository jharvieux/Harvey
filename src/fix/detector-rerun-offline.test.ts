...
// That path is the same `execFileSync`-throws branch with a different message, and MEASURED
...
describe("rerunDetector is notRun, never clean, when semgrep is not runnable (#1546)", () => {
  it("a harvey-* rule reports semgrep's absence rather than an unearned clean", () => {
    const dir = scratch("pages/api/redirect.js", "export default function handler(req, res) { res.redirect(302, req.query.u); }\n");
    const run = rerunDetector(finding({ taxonomy: HARVEY_RULE, location: "pages/api/redirect.js:1" }), dir);
  ...
  it("a registry-pack rule reports the REGISTRY-SPECIFIC reason, not just the shared failure", () => {
    const dir = scratch("components/LocationNav.jsx", 'export default () => { window.location = new URLSearchParams(location.search).get("to"); };\n');
    const run = rerunDetector(finding({ taxonomy: REGISTRY_RULE, confidence: "Review", location: "components/LocationNav.jsx:1" }), dir);
    expect(run.notRun).toContain("semgrep not found on PATH");
  ...
  it("NEGATIVE CONTROL: a detector that needs no binary still runs to a real verdict under the same mock", () => {
    ...
    const run = rerunDetector(finding({ taxonomy: "M5 — Unused parameter", location: "app/api/x/route.ts:1", severity: "Low" }), dir);
