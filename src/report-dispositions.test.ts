...
describe("provenance-bound re-audit comparisons #2136", () => {
  it("keeps AoP 55→719 producer/scope expansion separate from regressions and resolutions", () => {
    ...
    const prior = { ...current, findings: current.findings.slice(1, 56).map((f, i) => ({ ...f, id: `old-${i}` })), auditContext: context({ engagementId: "old-audit", schemaVersion: "legacy", producerVersions: { scanner: "legacy" }, assessedScope: ["source"] }) };
    const doc = applyBaseline(current, prior);
    expect(doc.baseline?.comparison?.kind).toBe("tool-change");
    expect(doc.baseline?.counts).toEqual({ new: 0, resolved: 0, persistent: 55 });
    expect(doc.baseline?.comparison?.denominators).toMatchObject({ prior: 55, current: 719, matched: 55, comparableCurrent: 0, comparablePrior: 0, unresolvedCurrent: 664 });
    expect(baselineLedger(current.findings, doc.findings).ok).toBe(true);
    const html = buildHtml(doc);
    expect(html).toContain("Prior population 55; current population 719; matched evidence 55");
  ...
  it.each(["missing", "same-source", "scope-change", "target-change"])("does not attribute unmatched observations to source changes with %s context", (mode) => {
    ...
    const options = mode === "missing" ? {} : { ...comparison, currentContext: mode === "same-source" ? { ...comparison.currentContext, target: comparison.priorContext.target } : mode === "scope-change" ? { ...comparison.currentContext, assessedScope: ["new-scope"] } : { ...comparison.currentContext, target: { id: "other", revision: "current" } } };
