    ...
    entries: [
      {
        ...
        note: "#215 Medium: checkoutWithStripe (utils/stripe/server.ts:69,76 + utils/helpers.ts:52-68) trusts the client-supplied `price` object and re-derives trial_end from price.trial_period_days — a client can request an arbitrarily long free trial (the charge amount itself is not forgeable; Stripe validates the price id). MEASURED 2026-07-24: uncaught. Its planted twin (P-CLIENT-PAYMENT-AMOUNT, b14-applogic.entries.ts) is a literal `amount: req.body.amount` grep shape; this real call re-derives a field from a passed-in object with no req.* literal at the call site — the m9-authz.entries.ts header already names this exact gap ('the price/trial variant needs \"should this field be re-read server-side?\" business context no AST pass has').",
      },
    ],
  },
];
