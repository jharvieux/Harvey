...
export const owaspReactEntries: CorpusEntry[] = [
  ...
  },
  {
    ...
    expectedTier: "review",
    expectedSeverity: "High",
    note: "#1544 planted this as the ONE row the destructuring requirement costs (full history in that entry's prior note, preserved in git blame) — modelled on carbon (crbnos/carbon@92e19c0) apps/erp/app/modules/production/ui/Jobs/JobBillOfProcess.tsx:444 → :655, `const [params] = useUrlParams()` reaching `storage.from('private').upload(...)`. #1708 CLOSES THE GAP: harvey-path-traversal's `pattern-sources` now carries `*dom_source` alongside `*request_source` (src/scan/rules/semgrep/injection.yml) because its Supabase Storage sink is a supabase-js call reachable directly from a client component — the exact mechanism this fixture models. Re-tiered from `expectedTier: \"none\"`/`gapKind: \"measured-gap\"` to a normal review-tier positive, matching the WARNING/harveySeverity-High the rule already carries; `neighbourTaxonomies` dropped because it is meaningless off a `none` row (misplacedNeighbourDeclarations, src/scan/calibration/graduation.ts) and this row no longer needs a graduation-guard exclusion list — it is graduated. MEASURED 2026-07-31: `semgrep --config src/scan/rules/semgrep/injection.yml` on this fixture now reports `harvey-path-traversal:30`, none before this change.",
  },
  {
