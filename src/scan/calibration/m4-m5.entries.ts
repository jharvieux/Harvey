...
export const m4m5Entries: CorpusEntry[] = [
  ...
  { id: "M4-P-CLONE-A", kind: "positive", cls: "Copy-pasted tax/rounding block", location: "invoice-total.ts", expectedTier: "high", note: "dup/invoice-total.ts and dup/order-total.ts share a genuine 27-line copy-pasted tax/rounding block (299 tokens). jscpd clone cluster; jscpdToFindings emits an M4-* finding at Low severity (15-49 duplicated lines)." },
  { id: "M4-P-CLONE-B", kind: "positive", cls: "Copy-pasted report aggregation block", location: "report-a.ts", expectedTier: "high", note: "dup/report-a.ts and dup/report-b.ts share a 52-line copy-pasted metric-aggregation block (658 tokens). jscpd clone cluster; jscpdToFindings' severityForClone -> Medium (>=50 lines)." },

  // --- M4 duplication NEGATIVES ---
  { id: "M4-N-GENERATED", kind: "negative", cls: "Generated file repeating a block found elsewhere", location: "schema.gen.ts", note: "dup/generated/schema.gen.ts repeats the invoice-total.ts tax block but is a generated file, not a hand-maintained duplicate. Excluded via the quality-scan CLI's jscpd --ignore glob (extended to **/generated/**, src/cli/quality-scan.ts) — jscpd never sees it." },
  { id: "M4-N-BOILERPLATE", kind: "negative", cls: "Framework-mandated route boilerplate", location: "route-a.ts", note: "dup/route-a.ts and dup/route-b.ts share the Next.js API-route config+handler-signature boilerplate. The shared span stays under jscpd's 50-token minimum (.jscpd.json minTokens) — not a defect, not flagged." },

  // --- M5 dead code (knip) POSITIVES ---
  { id: "M5-P-DEAD-EXPORT", kind: "positive", cls: "Unused named export", location: "dead/orphan.ts", match: ["unusedhelper"], expectedTier: "high", note: "dead/orphan.ts exports unusedHelper(), imported nowhere (dead/consumer.ts only uses computeTotal). knip issues[].exports; knipToFindings -> M5-*." },
  { id: "M5-P-DEAD-FILE", kind: "positive", cls: "Unreferenced file", location: "dead/never-imported.ts", expectedTier: "high", note: "dead/never-imported.ts is never imported by any entry point. knip top-level files[]; knipToFindings -> M5-* (measured line count)." },

  // --- M5 dead code NEGATIVES ---
  { id: "M5-N-NEXT-MAGIC", kind: "negative", cls: "Next.js framework-magic exports", location: "pages/dead-page.js", note: "pages/dead-page.js's default export + getServerSideProps are called by Next.js routing/build convention, never by import. knip's built-in Next.js plugin (auto-detected via the target's `next` dependency) treats pages/**/*.{js,jsx,ts,tsx} as entry points — not flagged." },
  { id: "M5-N-DYNAMIC-REF", kind: "negative", cls: "String-lookup dynamic export", location: "dead/registry.ts", note: "dead/registry.ts's handlerA is referenced only via dead/dispatch.ts's handlers[name] runtime lookup, invisible to static analysis. Tagged @lintignore; knip.json's tags:[\"-lintignore\"] silences it — not flagged." },
