...
export const owaspMultiTenantEntries: CorpusEntry[] = [
  ...
  },
  {
    ...
    expectedTier: "review",
    expectedSeverity: "High",
    note: "Our OWASP/CheatSheetSeries#2309 item 2. The sheet's own policy reads current_setting('app.current_tenant') and never mentions SET LOCAL or transaction-mode pooling. `SET app.current_tenant = …` on a pooled connection persists after release, so a later request is evaluated against the PREVIOUS tenant's id — a cross-tenant read in which policy, schema and application code all still look correct. WAS a measured gap (MEASURED 2026-07-26: zero findings). CLOSED by #1195's tenant-guc-scope.ts — a line-scoped grep for a query call whose SET/set_config text names a tenant/org GUC with no LOCAL / no third `true` argument. Review tier: it cannot see whether the pool is actually in transaction mode, only that the syntax is the risky one. Tracked in #1195. THE SECOND DEFECT ON THE SAME LINE, and the masking question #1195 asked but nobody recorded the answer to: `SET app.current_tenant = '${tenantId}'` also string-interpolates its argument into SQL, so does an injection rule fire here and stand in for the isolation defect? MEASURED 2026-07-27 (raw mechanical scan over targets/calibration, `quick-scan --dir targets/calibration --findings-out`): EXACTLY ONE finding lands on this file, TENANT-guc-set-not-local at line 13, and NO injection rule fires. So there is no masking — this row is satisfied by the defect it is testing. The reason the injection rules are silent is correct behaviour, not a second gap: they are taint-mode with REQUEST sources (req.query/body/params/cookies/headers, searchParams, await req.json()), and `tenantId` here is an ordinary function parameter with no request source in the file, so no taint path exists to report. An engagement where the same interpolation is fed straight from a request WOULD be reachable by them.",
  },
  {
