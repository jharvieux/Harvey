...
export const b23D091GapEntries: CorpusEntry[] = [
  ...
  },
  {
    ...
    expectedTier: "review",
    note: "D-091 item 27, SHIPPED by #1352 (src/scan/idempotency.ts, `webhookOrdering`) after the FP measurement the issue asked for FIRST. MEASURED 2026-07-31 by running the candidate predicate over all 17 pinned EXTERNAL_CORPUS clones (13,113 loadable source files): 61,346 functions with a first identifier parameter, 176 whose `.update`/`.upsert` payload derives from it, 6 of those in a webhook-shaped path, 0 whose parameter carries an ordering field. So the corpus produced no false positives AND no true positives — an FP rate over zero firings is undefined, not zero, and the emitted finding says exactly that instead of implying a clean field reading. The measurement earned one narrowing before the rule shipped: 2 of the 6 webhook-path rows were `crypto.createHmac(…).update(rawBody)`, a signature digest rather than a database write, so the write must resolve to a Supabase `.from(…)` table or a prisma-style model call. The former by-design label and its reach argument are GONE (#1350/#1352): four sibling detectors prove an absence with the same reach problem and ship at review tier with the bound stated in the finding, so reach was never what decided this item.",
  },
];
