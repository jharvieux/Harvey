...
export const b15NextjsAuthzEntries: CorpusEntry[] = [
  ...
  { id: "P-MW-SOLE-AUTHZ", kind: "positive", cls: "middleware is the sole authz layer — no defense-in-depth re-check in the route/data layer", location: "pages/api/admin/dashboard.js", match: ["mw-sole-authz"], expectedTier: "review", note: "#133: pages/api/admin/dashboard.js reads admin_metrics with no session/role check of its own, relying entirely on middleware.ts. It's a SELECT (not insert/update/delete), so the existing harvey-route-noauth mutation rule does not fire here — intentional, keeps this class isolated. Needs defense-in-depth/control-flow reasoning (roadmap §4a: middleware-sole-authz-layer, scan-gaps §2.1) — no mechanical rule. Expected to stay uncaught in the offline gate." },
  ...
  { id: "P-HOST-HEADER-URL", kind: "positive", cls: "Host header trusted to build an absolute URL (reset-link poisoning)", location: "lib/reset-link.js", match: ["host-header-url"], expectedTier: "review", note: "#135: lib/reset-link.js builds the password-reset link from headers().get('host') — an attacker-controlled value — so a spoofed Host redirects the real reset token to the attacker's domain. Needs taint-from-Host-into-URL reasoning — roadmap §4a: host-header-trusted-in-url-construction, no mechanical rule. Expected to stay uncaught in the offline gate." },
  { id: "N-HOST-HEADER-FIXED-ORIGIN", kind: "negative", cls: "reset link built from a fixed allowlisted origin, not the Host header", location: "lib/reset-link-safe.js", match: ["host-header-url"], note: "#135: lib/reset-link-safe.js builds the link from process.env.NEXT_PUBLIC_SITE_URL — headers()/Host is never read. Cleared trivially (no mechanical rule); recorded to complete the corpus pair." },
];
