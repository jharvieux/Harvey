...
// (no code egress), degrading silently (no finding, no throw) on network failure or a non-OK
// status — indeterminate is not the same as "no license". `licenseTier` on each returned finding
// is "high" for a copyleft match (the SPDX id itself is deterministic, self-declared registry
// data) and "review" for an unknown/missing/UNLICENSED license (the `license` field can be simply
// absent from otherwise-fine metadata, so it needs a human look before treating it as a real gap).
