...
export interface ModuleBaseline {
  ...
  total: number;
  // #1485 — this target/module pair is a FALSE-POSITIVE FLOOR: its job in the corpus is to stay
  // silent, so that a widening rule lights it up and fails the drift run. Until this flag the role
  // was written in prose only, and #1485 found the M7 floor sitting at 0% precision — a negative
  // control that had already saturated could no longer fail in the direction it was built to fail
  // in. `fpFloorViolations` makes the role machine-checked: a declared floor with `counted > 0` is
  // a hard failure, which is what closes the escape hatch the issue names ("do not resolve this by
  // removing the target's graded rows from the baseline"). Raising the baseline to absorb a new
  // graded row is exactly what now fails.
  fpFloor?: true;
  note: string;
}

// #1485 criterion 3, second half. The flag above is only as good as the reconciliation between it
// and the PROSE, because "this target is the FP floor" has been written in notes since #360 and a
// note can claim the role without the flag — which is the convention the issue asks to replace.
// So every note that CLAIMS a floor must either carry `fpFloor: true` or be triaged below.
//
// This net is VOCABULARY-BOUNDED and says so wherever its count is printed (the #1411 lesson,
// applied here rather than repeated): it sees the phrases this repo has actually used, and a note
// that invents a new way to say "floor" is invisible to it. It is a ratchet, not a census.
const FLOOR_CLAIM_PHRASES: readonly RegExp[] = [
  /\bFP FLOOR\b/,
  /\bFP floor\b/,
  /\bFP-floor\b/,
  /\bFP guard\b/,
  /\bfalse-positive floor\b/i,
  /\bfalse-positive regression guard\b/i,
  /\bfloor role\b/i,
];

export type FloorClaim = { slug: string; module: string; counted: number };

// Every baseline whose NOTE claims a floor role, flagged or not — the population the triage below
// has to account for.
export function floorClaimingNotes(corpus: readonly ExternalTarget[] = EXTERNAL_CORPUS): FloorClaim[] {
  const out: FloorClaim[] = [];
  for (const t of corpus) {
    for (const [module, b] of Object.entries(t.modules)) {
      if (!b || isMutationBaseline(b) || !("counted" in b)) continue;
      if (FLOOR_CLAIM_PHRASES.some((re) => re.test(b.note))) out.push({ slug: t.slug, module, counted: b.counted });
    }
  }
  return out;
}

// The failing direction the whole flag exists for: a declared floor that is no longer silent.
export function fpFloorViolations(corpus: readonly ExternalTarget[] = EXTERNAL_CORPUS): FloorClaim[] {
  const out: FloorClaim[] = [];
  for (const t of corpus) {
    for (const [module, b] of Object.entries(t.modules)) {
      if (!b || isMutationBaseline(b) || !("counted" in b) || !b.fpFloor) continue;
      if (b.counted !== 0) out.push({ slug: t.slug, module, counted: b.counted });
    }
  }
  return out;
}

// #1485 criterion 4 — the audit, recorded rather than re-derived by the next reader. Every note
// that claims a floor but does NOT carry the flag, with the reason it does not. MEASURED
// 2026-07-31 by `floorClaimingNotes` over the pinned manifest: 34 claims, 26 real floors (flagged),
// these 8. Two shapes recur and they are different: a phrase used to DENY the role, and a role that
// was real and has since saturated. The second shape is #1485's own finding occurring elsewhere,
// so it is named here instead of being left for a future reader to rediscover.
export const FLOOR_CLAIM_TRIAGE: readonly { readonly key: string; readonly disposition: string }[] = [
  { key: "proposit:M1-boundary", disposition: "NOT a floor claim — the phrase appears inside a sentence denying it: 'a RECALL floor, not an FP floor'. 4 rows, 3 confirmed real and 1 out of static reach and stated as such." },
  { key: "subscription-payments:M9", disposition: "SATURATED floor — the note calls it 'an FP guard alongside M7' while carrying 2 graded rows, the same state #1485 found in this target's M7. Rows unread; triage tracked in #1773, so the role is not re-declared here on an untriaged baseline." },
  { key: "mvp-boilerplate:M4-diverged", disposition: "SATURATED floor — declared 'Same FP-floor role as this target's M7/M5-slop zeros' at 0, moved to 1 by #1095/PR#1129's same-file-pair widening. Rows unread; tracked in #1773." },
  { key: "ghostfolio:M4-diverged", disposition: "SATURATED floor — 'A useful FP floor on a large repo' at 0, moved to 2 by the same #1095 widening. Rows unread; tracked in #1773." },
  { key: "carbon:M4-diverged", disposition: "SATURATED floor — 'The strongest FP-floor reading in the corpus' at 0, moved to 3 by the same #1095 widening. Rows unread; tracked in #1773." },
  { key: "boxyhq:M5-knip", disposition: "NOT a zero floor — the note's claim is 'worth watching as an FP guard: a jump here is more likely a knip/config change than new dead code', i.e. a DRIFT watch on a nonzero baseline, which the drift check already enforces." },
  { key: "launch-mvp:M5-knip", disposition: "NOT a zero floor — same drift-watch claim ('worth watching as an FP guard if the count moves'), on a baseline that was never asserted to be silent." },
  { key: "supabase-security-labs:M4", disposition: "NOT a zero floor — the note hedges it explicitly as 'A near-floor reading' and 'at or near zero BY CONSTRUCTION', with 1 counted Low." },
];
...
export const EXTERNAL_CORPUS: ExternalTarget[] = [
  ...
  },
  {
    ...
    modules: {
      ...
      "M5-slop": { counted: 17, total: 19, note: "#1948 HOSTED REPLAY run 32345629796: RE-MEASURED 2026-08-20 at 17/19 (was 14/16), +3 graded rows from newly registered exception-flow/type-escape detectors: 2 empty catch + 1 unexplained `@ts-ignore`. Existing M5 categories were unchanged. Prior note: #278 started this split at a measured 10; re-measured 2026-07-17 after #391's new classes: 9 'Else after return' + 4 'Single-use helper' + 1 'Single-call wrapper' counted; 2 'Decorative emoji' Info." },
      ...
      M9: { counted: 2, total: 3, note: "#1460/#1461/#1462: RE-MEASURED 2026-07-28 at 2/3 (was 4/5) — DRIFT -2, same single cause as proposit's -1, both rows READ: utils/stripe/config.ts:4 and utils/supabase/admin.ts:17, each reachable only as CustomerPortalForm.tsx ('use client') -> utils/stripe/server.ts ('use server') -> the module. Traversing a `\"use server\"` boundary is not a client bundle path. MEASURED via `pnpm exec tsx src/cli/static-detect.ts <clone>` on the identical pinned clone, pre- and post-change arms. Prior note retained below. PRIOR: #1262/#1292: RE-MEASURED 2026-07-28 at 4/5 (was 4/4) — the disclosure row only, no detection change. MEASURED by a controlled before/after `detect-static` on the identical pinned clone, one variable (this PR's M9 diff): the before arm at 564bded reprints 4/4. The +1 total is #1262's `M9-RETRY` scope row — a counted not-assessed row disclosing the three uncapped retry/fan-out sub-shapes the new AST detector does not reach, emitted on any target that has route/edge handlers at all. Info, so `counted` is unchanged. Prior note: Re-measured 2026-07-17: 2->4 — the 2 'Accidental dynamic rendering' plus 2 'Missing server-only guard' from #380/#381's new M9 classes. Still a small stable surface and an FP guard alongside M7. #1485/#1773: this note's FP-FLOOR claim is SATURATED and is recorded as such in FLOOR_CLAIM_TRIAGE — the baseline is no longer silent, so the row can no longer fail in the direction a floor exists to fail in. Whether the rows are true or false is UNREAD; triage tracked in #1773." },
  ...
  // for.
  {
    ...
    modules: {
      ...
      "M5-slop": { counted: 20, total: 22, note: "#1948 HOSTED REPLAY run 32345629796: RE-MEASURED 2026-08-20 at 20/22 (was 6/8), +14 graded rows from newly registered exception-flow/type-escape detectors: 5 log-only catch + 9 unexplained `@ts-ignore`. Existing M5 categories were unchanged. Prior note: #1372: MEASURED 2026-07-28 — 3 'Single-use helper', 1 'Orphan TODO' + 1 'Placeholder stub' (both app/verify-email/page.tsx:34, the same unfinished handler), 1 'Unused import'; 2 Info (1 decorative emoji, 1 narrating comment). A small, clean reading for a solo-maintained template." },
  ...
  },
  {
    ...
    modules: {
      ...
      "M5-slop": { counted: 22, total: 29, note: "#2132 SHARED PRODUCT INVENTORY: measured 2026-09-20 at scanner f08a5041dca73d35e1ac863be7abe442e0daf497 on target pin 2aac5c2fcb45c35aa4a5f5eb9eb66645f0f84e70 via `node --import tsx src/cli/static-detect.ts <pin> --out findings.json --scope-out scope.json`: 22 counted / 29 total (previous committed 31/36). PRECISION CHANGE: nine graded rows came only from the evidenced inactive monero/patches install overlay: six single-use helpers (AccountPage statusVariant plus five shared Supabase helpers), two else-after-return rows, and one Pricing double assertion. monero/install.sh backs up the existing product files then copies the staged replacements; docs/monero/setup.md requires this explicit installation, and the pinned base has no installed marker. Seven shared-Supabase rows and the Pricing assertion remain reported in their live destination files; statusVariant belongs only to the optional variant. An in-memory control using unchanged loader/detector code and removing only that overlay boundary restores exactly those nine M5 rows, with no lost retained rows. It reads 31/38, matching prior successful hosted main run 35467879886, whose two source-coverage not-assessed Info disclosures for c and cpp were already absent from the stale committed 31/36 total. These are not new emissions in this change; they remain in the complete measured 22/29 total. This is a scope correction, not a claim that optional Monero source is defect-free. Historical measurement: #1948 HOSTED REPLAY run 32345629796: RE-MEASURED 2026-08-20 at 31/36 (was 24/29), +7 graded rows from newly registered exception-flow/type-escape detectors: 4 empty catch + 1 log-only catch + 2 double assertions. Existing M5 categories were unchanged. Prior note: #1533: RE-MEASURED 2026-07-31 at 24/29 (was 23/28) — DRIFT +1, a RECOVERED FALSE NEGATIVE, not a new over-match. The row is the one the previous note enumerated as the disclosed `async`-caller-with-no-await boundary: `generateGameOfLifePattern` (nextjs/app/api/og/route.tsx:15), a pure grid computation whose caller `GET` is declared `async`, contains ZERO awaits and returns `new ImageResponse(...)` — constructed synchronously, so the exemption's premise (\"the caller does the I/O\") is false. #1533 reads the caller's RETURNS through the cross-file resolver `buildImportGraph` is built on and fires only when every one of them is provably synchronous, so the sibling shape that stopped this being narrowed before — inbox-zero utils/outlook/mail.ts:408, whose await-free caller returns a local `async` function's promise — is still correctly spared. This target's baseline is now the SAME number it read before #1532/#1447 (24/29), reached by a different and narrower route. Command: `pnpm exec tsx src/cli/static-detect.ts <clone-of-2aac5c2f> --out f.json`. Prior note: #1532: RE-MEASURED 2026-07-30 at 23/28 (was 24/29) — DRIFT -1, a PRECISION FIX taken against a NARROWED scanner. Same cause and method as proposit's M5-slop note (#370/#1447's `isTestabilitySeam`; #1532's `doesOwnIo` narrows it). Prior note: Re-measured 2026-07-17: 6->24 counted after #391's new classes — 17 'Single-use helper' + 5 'Else after return' + 1 'Orphan TODO' + 1 'Unused import'; 5 'Decorative emoji' Info." },
  ...
  },
  {
    ...
    commit: "63892cee72ffc9885bd5bf366ed6f4be78aab1ea",
    license: "MIT",
    provenance: "professional",
    ...
    m8: M8_CORPUS_CONFIGS["saas-lite"],
    modules: {
      "M1-boundary": {"counted":0,"total":0,"fpFloor":true,"note":"#1818: RE-MEASURED ZERO 2026-08-04 against refreshed Saas-Lite pin 63892cee via corpus-drift. The high-impact missing-auth/client-owner-id floor remains silent; any graded row is still a hard FP-floor failure."},
      "M4": {"counted":50,"total":57,"note":"#2132: RE-MEASURED 2026-09-20 at 50 counted / 57 total on unchanged Saas-Lite pin 63892cee. Hosted run 35487479299 at Harvey 18226595 adds exactly one Medium same-file clone: apps/web/app/home/_components/dashboard-demo-charts.tsx:705-797 versus 505-597 (93 lines, 2097 tokens; 91 identical date/value objects). The active /home page renders both charts; removing filename-only demo exclusions recovers authored source, not an inactive/generated fixture. Controlled immutable-main/current/restored-demo-glob/repeated-current Jscpd runs reproduce 49/50/49/50 counted with unchanged remaining identities. Prior main run 35467879886 already emitted 49/56, so the old total 55 was stale by one; current adds one counted row and preserves all seven Info rows, with scope disclosures updated. Prior note: #1818: RE-MEASURED 2026-08-04 at 49/55 against refreshed Saas-Lite pin 63892cee via pnpm corpus-drift --target saas-lite --install. This is upstream corpus evolution from the former 17/22 pin, not scanner drift on identical source."},
      "M4-diverged": {"counted":4,"total":4,"note":"#1818: RE-MEASURED 2026-08-04 at 4/4 against refreshed Saas-Lite pin 63892cee. Security-path near-duplicate families remain fully reported; the former pin measured 5/5."},
      "M5-knip": {"counted":38,"total":38,"note":"#1818: RE-MEASURED 2026-08-04 at 38/38 against refreshed Saas-Lite pin 63892cee with the pnpm-aware install path. All workspace scopes were accounted for by corpus-drift."},
      "M5-slop": {"counted":70,"total":73,"note":"#1948 HOSTED REPLAY run 32345629796: RE-MEASURED 2026-08-20 at 70/73 (was 68/71), +2 graded rows from newly registered exception-flow detectors: 1 ambiguous exception success + 1 empty catch. Existing M5 categories were unchanged. Prior note: #1818: RE-MEASURED 2026-08-04 at 68/71 against refreshed Saas-Lite pin 63892cee. The change from 73/76 is upstream corpus evolution; this remains the corpus drift watch for helper/JSDoc granularity."},
      "M6-indicator": {"counted":4,"total":4,"note":"#1818: RE-MEASURED 2026-08-04 at 4/4 against refreshed Saas-Lite pin 63892cee. M6 indicator rows are Info/non-grading and counted equals total by construction."},
      "M7": {"counted":5,"total":6,"note":"#1818: RE-MEASURED 2026-08-04 at 5/6 against refreshed Saas-Lite pin 63892cee. This records the full performance probe output, including its Info disclosure row."},
      "M8": {"mutationScore":78.77,"killed":115,"valid":146,"coveredScope":["src/hooks/use-async-dialog-state.ts","src/lib/utils/is-route-active.ts"],"note":"#1818: MEASURED 2026-08-04 with Stryker 9.6.1 over the two new packages/ui unit-tested files: 115 killed, 25 survived, 6 NoCoverage, 146 valid; 78.77% overall and 82.14% over covered code. Scoped subset, not a whole-repo coverage claim. Harvey bypassed the TypeScript-7-incompatible tsconfig preprocessor resolved by Stryker core while leaving the target TypeScript 7/Vitest toolchain intact. Line-coverage auto-pull remained partial because @vitest/coverage-v8 is not installed; that limitation was disclosed separately in the M8 artifact."},
      "M8-intent": {"counted":1,"total":1,"note":"#1818: RE-MEASURED 2026-08-04 at 1/1 against refreshed Saas-Lite pin 63892cee. This static test-intent result is separate from the now-measured Stryker mutation score."},
      "M9": {"counted":4,"total":5,"note":"#1818: RE-MEASURED 2026-08-04 at 4/5 against refreshed Saas-Lite pin 63892cee. The former pin measured 6/7; this is upstream corpus evolution, not a same-source scanner rebaseline."},
      "M10": {"counted":1,"total":1,"note":"#1818: RE-MEASURED 2026-08-04 at 1/1 against refreshed Saas-Lite pin 63892cee using apps/web/supabase/migrations."},
    },
  },
  ...
  // top of this file.
  {
    ...
    modules: {
      ...
      "M8-intent": { counted: 0, total: 0, fpFloor: true, note: "#894: MEASURED zero 2026-07-24 across this target's 31 spec files — no assertion-free/tautological/happy-path-only/mock-the-subject shape fired. A real FP floor for the test-intent pass on a professionally-maintained suite, and the direct contrast with inbox-zero's 305." },
  ...
  },
  {
    ...
    modules: {
      ...
      M9: { counted: 2, total: 7, note: "#1502: RE-MEASURED 2026-07-31 at 2/7 (was 5/10) — DRIFT -3, a PRECISION FIX, and the CORPUS-WIDE re-measurement #1502's own criterion 4 (which names only carbon and tanstack-com) did not reach. The three cleared rows are apps/web/src/features/calendars/client.tsx:19, :23 and :28, all `M9 — SSR-only API misuse`, all three `window.location.*` reads inside the single exported arrow `connectToCalendar`. READ AGAINST SOURCE, not inferred from the count: that helper's ONLY caller anywhere in the tree is app/[locale]/(space)/settings/calendars/components/connect-calendar-dropdown.tsx (imported at :13, called at :27 and :40), and BOTH call sites sit inside `onClick={() => …}` handlers on `DropdownMenuItem` — browser-only event callbacks, never invoked during a render pass. `\"use client\"` on the defining module is NOT what clears them (a client component is still server-rendered on first request); what clears them is that #1502's cross-file call-site resolution can now see that no call site is on a render path. All three are genuine false positives. NOT a regression: the fail-safe half is intact — a helper whose callers cannot be resolved stays flagged, which is what `M9C-SSRXFILE-POS` scores. CONTROLLED before/after on the identical pinned clone (a680798c), one variable (this PR's diff): `pnpm exec tsx src/cli/static-detect.ts <clone> --out f.json` reads 226 findings / 10 M9 rows / 5 counted on a pristine-main arm at 1cb4a32 and 223 / 7 / 2 here, with an exact location-level diff of 3 only-before and ZERO only-after — nothing moved anywhere else, in any module. BLAST RADIUS re-measured across ALL 17 pinned targets rather than assumed from the two named in the issue: the sharded corpus-drift run on this PR head scored 1 + 8 + 8 = 17 targets and every other baseline on every other target matched (carbon M9 104 and M1-boundary 6; tanstack-com M9 4 and M1-boundary 0; proposit 10, subscription-payments 2, boxyhq 0, launch-mvp 0, saas-lite 6, cravab 9, flori-web 19, multi-tenant-starter 2, mvp-boilerplate 1). rallly is the only baseline the mechanism moves corpus-wide. Prior note retained below. PRIOR: #1460/#1461/#1462: RE-MEASURED 2026-07-28 at 5/10 (was 12/17) — DRIFT -7, every one in the #1460 SSR helper family, all 7 in packages/ui/src/event-calendar/event-calendar-dnd.tsx (module-level pointer-gesture helpers reached only from listener callbacks). event-calendar-dnd.tsx:86 REMAINS, which is the scope control on the same file. This target also exercised two defects #1461's alias fix EXPOSED rather than caused, both fixed in this PR and neither visible in the final number: a type-only `import type { AppRouter }` in the 'use client' tRPC provider made the whole server router tree read as client-reachable (3 spurious `Missing server-only guard` Highs), and the reachability walk crossed `\"use server\"` boundaries (the remaining 2). Net M9 `Missing server-only guard` here is 0 before and after. MEASURED via `pnpm exec tsx src/cli/static-detect.ts <clone>` on the identical pinned clone, pre- and post-change arms. Prior note retained below. PRIOR: #1262/#1292: RE-MEASURED 2026-07-28 at 12/17 (was 12/15) — +2 total, NEITHER of them a detection change. MEASURED by a controlled before/after `detect-static` on the identical pinned clone, one variable (this PR's M9 diff): the before arm at 564bded reprints 12/16. Read the arithmetic carefully: the before arm on this PR's base commit already reads 12/16, not the 12/15 recorded here — a PRE-EXISTING undisclosed drift of +1 Info that this PR did not cause and is correcting in passing. Its cause is #1051's `M9 — Cross-user cache bleed — not assessed` row, which joined the three #903 not-assessed rows on this non-Supabase target and was never folded into this baseline. The +1 total is #1262's `M9-RETRY` scope row — a counted not-assessed row disclosing the three uncapped retry/fan-out sub-shapes the new AST detector does not reach, emitted on any target that has route/edge handlers at all. Info, so `counted` is unchanged. Prior note: #964: RE-MEASURED 2026-07-24 (this PR) at 12 counted / 15 total (3 Info not-assessed rows) — the #964 M9 SSR precision fix dropped 2 'SSR-only API misuse' FPs: a `document.createElement` read in the non-component `.ts` util apps/web/src/lib/image-processing.ts and a `navigator.userAgent` read in the non-component `.ts` hook packages/ui/src/hooks/use-platform.ts. Residual 12: 11 SSR-only + 1 accidental dynamic rendering (cookies() read). Prior note: #894: MEASURED 2026-07-24 — 13 SSR-only API misuse (11 window, 1 document, 1 navigator) + 1 'cookies() read in a page'." },
  ...
  // docs/design/carbon-scale-measurement.md; these are the baselines that completed.
  {
    ...
    modules: {
      ...
      "M8-intent": { counted: 5, total: 5, note: "#1948 HOSTED REPLAY run 32345629796: RE-MEASURED 2026-08-20 at 5/5 (was 3/3), +2 `M8 — Production-independent assertion` findings in packages/checks/src/smoke.test.ts:5 and packages/harness/src/smoke.test.ts:5. Existing M8-intent categories were unchanged. Prior note: #897: MEASURED 2026-07-24 — 1 'Call-count-only test' (Low) and 2 'Happy-path-only tests on security/money-critical code' (Medium: no-legacy-rls.ts, build-payment-journal.ts). A striking ratio: 77 test files across 4,110 source files produce almost no test-intent signal, because there is barely any test surface to inspect." },
  ...
  //      adversarial positives, not a rushed widening on the day the FP was found.
  {
    ...
    modules: {
      "M1-boundary": { counted: 0, total: 0, note: "#1500: RE-MEASURED 2026-07-31 at 0/0 (was 6/6) — DRIFT -6, the whole family cleared, and every one of the 6 was a recorded FALSE POSITIVE (see #1459's prior note, preserved below), so the drop is the whole point rather than a loss. Command: `pnpm exec tsx src/cli/static-detect.ts <clone-of-6b61f4dc>`, controlled before/after with one variable (the #1500 diff): before 687 findings incl. the 6 `M1 — server function missing authorization check` rows, after 681, 0 rows only-after — nothing moved anywhere else in the corpus. Two independent fixes were both necessary (each proven load-bearing by reverting it alone and watching the corpus gate `M9C-GATEDEPTH-NEG` go red): GATE_DEPTH raised 2 -> 4 (the real check sits 4 resolvable hops from the action, not 2), and `GateResolver` now follows a binding introduced by a helper that WRAPS a dynamic import (`const { x } = await loadY()` where `loadY` is `() => import('literal')`, suppression-only) plus one hop of `export { x } from './barrel'` re-export resolution — `~/auth/index.server` re-exports `getAuthGuards` rather than declaring it. A THIRD, unnamed-by-the-issue fix was needed for the first two to reach the target at all: `requireCapability`'s body calls `getAuthGuards().requireCapability(...)`, a same-named self-call that — unguarded — resolved back to its own enclosing declaration, burning a depth level and pre-seeding `seen` before the real hop to `getAuthGuards` was reached; `firstCallee` now skips a resolution that returns the frame already open one level up. Adversarial positive `M9C-GATEDEPTH-POS` (four resolvable hops, none of which checks or denies anything) still fires, proving the widening only suppresses a REAL gate. PRIOR (#1459, 2026-07-28): 6 High rows, down from 12 before #1462's direct dynamic-import resolution. TRIAGE STATE at the time, stated per row: all 12 pre-#1462-fix rows were read against source and all 12 were FALSE POSITIVES. The 6 #1462 cleared were behind `requireAdmin()`, gated by a DIRECT `await import('./auth.server-helpers')`. The 6 that remained (showcase.functions.ts ×4, docFeedback.functions.ts ×2, now cleared by #1500) sat behind `requireModerateShowcases`/`requireModerateFeedback`, whose chain is action → require*() → requireCapability → (wrapper) getAuthGuards → (barrel) → createGuards, where the real check lives." },
      ...
      "M8-intent": { counted: 0, total: 0, fpFloor: true, note: "#1276: MEASURED ZERO 2026-07-28 across this target's 25 test files — no assertion-free / tautological / happy-path-only / mock-the-subject shape fired. An FP floor for the test-intent pass on a professionally-maintained suite, the same reading as ghostfolio's zero." },
      M8: {
  ...
  // manifest only, never vendored into targets/ and never distilled into a *.entries.ts.
  {
    ...
    modules: {
      ...
      "M4-diverged": { counted: 0, total: 0, fpFloor: true, note: "#895: MEASURED zero 2026-07-24 — FP floor." },
      ...
      "M5-slop": { counted: 0, total: 4, fpFloor: true, note: "#895: MEASURED 2026-07-24 — zero counted, 4 'Decorative emoji in a comment' Info. FP floor." },
      "M6-indicator": { counted: 0, total: 0, note: "#895: MEASURED zero 2026-07-24. All Info/non-grading (#267); counted === total by construction." },
      M7: { counted: 0, total: 0, fpFloor: true, note: "#895: MEASURED zero 2026-07-24 — FP floor. Any M7 finding appearing on a repo of SQL and two 40-line Deno functions is almost certainly a new over-match." },
  ...
  // handrolled-frequency.ts already uses, so the two measurements agree on which tree they describe.
  {
    ...
    schemaPath: "supabase/migrations",
    modules: {
      "M1-boundary": { counted: 0, total: 0, fpFloor: true, note: "#1524: MEASURED ZERO 2026-07-30 via `pnpm corpus-drift --target cravab --install` — the M9 boundary pass runs and produces no missing-auth / client-owner-id row here. An FP floor, same posture as every other target's M1-boundary reading." },
      ...
      "M8-intent": { counted: 0, total: 0, fpFloor: true, note: "#1524: MEASURED ZERO 2026-07-30 — the 3 spec files trip no assertion-free/tautological/happy-path-only/mock-the-subject shape. A real FP floor for the test-intent pass on this target's thin suite." },
  ...
  },
  {
    ...
    schemaPath: "supabase/migrations",
    modules: {
      "M1-boundary": { counted: 0, total: 0, fpFloor: true, note: "#1680/#1681: RE-MEASURED 2026-07-31 by a controlled before/after `pnpm exec tsx src/cli/static-detect.ts <clone>` over all 17 pinned clones, ONE variable (the src/detectors/app-router.ts diff). Corpus-wide M1-boundary 16 -> 9; no target GAINED a row and no other module moved on any target (identical scores in both arms). Per-issue attribution measured with two further single-fix arms: #1680 alone accounts for 6 of the 7 removed rows (inbox-zero 5, flori-web 1), #1681 alone for 2 (carbon 1, flori-web 1) — flori-web's single row is reached by both. THIS TARGET: 1 -> 0, and the prior note's claim that this row was 'a real finding, not just an FP floor' is RETRACTED. Read against the pinned source in this change: the row is `src/lib/logger.ts:17`, `function isDuplicate(key: string)` — a NON-EXPORTED helper in a `'use server'` module (so #1680), whose only mutation-named call is `recentErrors.delete(k)` on a module-level `new Map()` used as an in-memory 5-minute dedup window (so #1681 as well). There is no database anywhere in the file. It was a false positive reachable by either fix, and it is the row that made the #1681 exclusion scan the whole module for built-in-collection bindings rather than only the action body. PRIOR (RETRACTED): #1524: MEASURED 2026-07-30 via `pnpm corpus-drift --target flori-web --install` — 1 High 'Server Action missing authorization check'. Unlike the other two #1524 targets this is non-zero: a real finding, not just an FP floor." },
      ...
      "M4-diverged": { counted: 0, total: 0, fpFloor: true, note: "#1524: MEASURED ZERO 2026-07-30 — no diverged security-path family found. An FP floor." },
      ...
      "M5-slop": { counted: 135, total: 148, note: "#1948 HOSTED REPLAY run 32345629796: RE-MEASURED 2026-08-20 at 135/148 (was 95/108), +40 graded rows from newly registered exception-flow/type-escape detectors: 15 ambiguous exception success + 17 empty catch + 7 log-only catch + 1 double assertion. Existing M5 categories were unchanged. Prior note: #1524: MEASURED 2026-07-30 — 89 'Single-use helper' counted (Low), plus small counts of 'Unused import'/'Else after return'/'Orphan TODO'; 11 'Decorative emoji' + 2 'Narrating comment' Info. 'Single-use helper' alone (89) is UNTRIAGED and dominates — the same per-entity-scaffolding vein #1532's `doesOwnIo` fix addressed corpus-wide (this target predates that measurement, not exempt from it)." },
      ...
      M9: { counted: 18, total: 19, note: "#1680/#1681: RE-MEASURED 2026-07-31 by a controlled before/after `pnpm exec tsx src/cli/static-detect.ts <clone>` over all 17 pinned clones, ONE variable (the src/detectors/app-router.ts diff). Corpus-wide M1-boundary 16 -> 9; no target GAINED a row and no other module moved on any target (identical scores in both arms). Per-issue attribution measured with two further single-fix arms: #1680 alone accounts for 6 of the 7 removed rows (inbox-zero 5, flori-web 1), #1681 alone for 2 (carbon 1, flori-web 1) — flori-web's single row is reached by both. THIS TARGET M9: 19 -> 18 counted, 20 -> 19 total. The removed row is the `Server Action missing input validation` sibling at the same `src/lib/logger.ts:17` location — the single High in the prior breakdown, and false for the same reason. What remains: 16 'Accidental dynamic rendering' (Medium) and 2 'SSR-only API misuse' (Low), plus the 1 Info M9-RETRY scope disclosure row. PRIOR: #1524: MEASURED 2026-07-30 — 16 'Accidental dynamic rendering' (Medium), 2 'SSR-only API misuse' (Low), 1 'Server Action missing input validation' (High) counted; 1 Info M9-RETRY scope disclosure row." },
  ...
  },
  {
    ...
    license: "MIT",
    provenance: "ai-assisted",
    provenanceNote: "#413/#1832: original joshcoolman/effective@5274467 had CLAUDE.md + Co-Authored-By: Claude, but higher-skill Effect TS and a thin schema. After upstream became inaccessible, the MIT-licensed recovery mirror pinned a synthetic root whose tree is byte-identical (tree 0307f493) without claiming unavailable ancestry. Large tree (~500k LOC via a vendored repos/effect reference copy, stripped from every scan below — see vendoredSubtrees); the app itself is ~47 files under src/.",
    securityVerdict: "NOT ASSESSED — #1524 baselines the source-tier QUALITY modules only, same posture as #894's ghostfolio/rallly/etc entries. No M1 semantic pass, no dynamic tier, and no disclosure has been filed. This field is deliberately not a clean bill of health.",
    schemaPath: "supabase/migrations",
    vendoredSubtrees: ["repos"],
    modules: {
      "M1-boundary": { counted: 0, total: 0, fpFloor: true, note: "#1524: MEASURED ZERO 2026-07-30 via `pnpm corpus-drift --target effective --install` (post `repos/` removal, see vendoredSubtrees) — an FP floor on a small app." },
      ...
      "M4-diverged": { counted: 0, total: 0, fpFloor: true, note: "#1524: MEASURED ZERO 2026-07-30 — FP floor." },
      ...
      "M5-slop": { counted: 18, total: 18, note: "#1948 HOSTED REPLAY run 32345629796: RE-MEASURED 2026-08-20 at 18/18 (was 12/12), +6 graded rows from newly registered exception-flow detectors: 2 ambiguous exception success + 4 empty catch. Existing M5 categories were unchanged. Prior note: #1524: MEASURED 2026-07-30 — 10 'Single-use helper' (src/features/core/server.ts x5, scripts/*.mjs x3, src/features/core/use-chat.ts, src/features/generate/server.ts) + 2 'Single-call wrapper' (src/app/api/turn/route.ts, src/proxy.ts), all Low. The corpus's smallest M5-slop reading, consistent with this target's thin ~47-file app tree." },
      "M6-indicator": { counted: 0, total: 0, note: "#1524: MEASURED ZERO 2026-07-30. All Info/non-grading (#267); counted === total by construction." },
      M7: { counted: 0, total: 0, fpFloor: true, note: "#1524: MEASURED ZERO 2026-07-30 — FP floor on a small app with no visible request-path perf anti-pattern." },
