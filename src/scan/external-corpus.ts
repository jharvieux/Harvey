...
export const EXTERNAL_CORPUS: ExternalTarget[] = [
  ...
  },
  {
    ...
    modules: {
      ...
      "M5-slop": { counted: 23, total: 28, note: "#1532: RE-MEASURED 2026-07-30 at 23/28 (was 24/29) — DRIFT -1, a PRECISION FIX taken against a NARROWED scanner. Same cause and method as proposit's M5-slop note (#370/#1447's `isTestabilitySeam`; #1532's `doesOwnIo` narrows it). The one spared row is on the DISCLOSED-not-narrowed boundary and is recorded here rather than silently accepted: `generateGameOfLifePattern` (nextjs/app/api/og/route.tsx:15) is a pure grid computation whose caller `GET` is declared `async` and contains ZERO awaits — it returns `new ImageResponse(...)`, which is constructed synchronously, so no I/O happens and the exemption's premise does not hold. It is spared anyway because the sibling shape is indistinguishable by AST: inbox-zero utils/outlook/mail.ts:408 has an equally await-free caller that DOES do its I/O, by returning a promise. Narrowing on the `async`-with-no-await axis would trade this false negative for that false positive. Tracked as #1533. Command: `pnpm exec tsx src/cli/static-detect.ts <clone-of-2aac5c2f> --out f.json`. Prior note: Re-measured 2026-07-17: 6->24 counted after #391's new classes — 17 'Single-use helper' + 5 'Else after return' + 1 'Orphan TODO' + 1 'Unused import'; 5 'Decorative emoji' Info." },
  ...
  },
  {
    ...
    modules: {
      ...
      "M5-slop": { counted: 74, total: 77, note: "#1532: RE-MEASURED 2026-07-30 at 74/77 (was 79/82) — DRIFT -5, a PRECISION FIX taken against a NARROWED scanner. Same cause and method as proposit's M5-slop note (#370/#1447's `isTestabilitySeam`; #1532's `doesOwnIo` narrows it and RECOVERS 1 finding here, so the pre-fix reading was 73). All 6 originally-spared rows were read at source. 5 hold — `getClassName` (pure className computation, caller `RootLayout` awaits i18n + the theme cookie), `matchUrlPattern`, `setRequestId`, `captchaTokenGetter`, `shouldIgnoreError` (pure error-message classifier, caller awaits `signInWithOtp`). 1 is the disclosed `async`-caller-with-no-await boundary: `getPaths` (apps/web/app/sitemap.xml/route.ts:24) builds a static path list for a `GET` that awaits nothing — same shape as mvp-boilerplate's, tracked as #1533. This target is the corpus's declared M5-slop FP-drift watch, so its residual is enumerated rather than summarised. Command: `pnpm exec tsx src/cli/static-detect.ts <clone-of-37def9c2> --out f.json`. Prior note: Re-measured 2026-07-17: 23->79 counted, mostly #391's new 'Single-use helper' class (56) on top of the prior 22 'Redundant JSDoc' + 1 'Orphan TODO'; 3 Info (narrating comment, decorative emoji). Still the corpus's highest slop count — a well-maintained starter kit whose JSDoc/helper granularity habits trip the detectors, the corpus's main M5-slop FP-drift watch." },
  ...
  },
  {
    ...
    modules: {
      "M1-boundary": { counted: 5, total: 5, note: "#1459: MEASURED 2026-07-28 via `pnpm exec tsx src/cli/static-detect.ts <clone>` over the pinned tree — 5 High rows. NOT TRIAGED (drift baseline, not a defect count)." },
