...
export const EXTERNAL_CORPUS: ExternalTarget[] = [
  ...
  },
  {
    ...
    modules: {
      ...
      M9: { counted: 0, total: 1, note: "#1262/#1292: RE-MEASURED 2026-07-28 at 0/1 (was 1/1) — the target's only counted M9 finding was a waterfall FALSE POSITIVE. MEASURED by a controlled before/after `detect-static` on the identical pinned clone, one variable (this PR's M9 diff): the before arm at 564bded reprints 1/1. The -1 is #1292: monero/supabase/functions/xmr_webhook/index.ts:114, a guarded pair. This target now reads 0 counted with the disclosure row as its only M9 output — a legitimate zero, stated rather than silent. The +1 total is #1262's `M9-RETRY` scope row — a counted not-assessed row disclosing the three uncapped retry/fan-out sub-shapes the new AST detector does not reach, emitted on any target that has route/edge handlers at all. Info, so `counted` is unchanged. Prior note: 1 'Data-fetching waterfall' (re-confirmed 2026-07-17)." },
  ...
  },
  {
    ...
    modules: {
      ...
      M9: { counted: 10, total: 11, note: "#1262/#1292: RE-MEASURED 2026-07-28 at 10/11 (was 10/10) — the disclosure row only, no detection change. MEASURED by a controlled before/after `detect-static` on the identical pinned clone, one variable (this PR's M9 diff): the before arm at 564bded reprints 10/10. The +1 total is #1262's `M9-RETRY` scope row — a counted not-assessed row disclosing the three uncapped retry/fan-out sub-shapes the new AST detector does not reach, emitted on any target that has route/edge handlers at all. Info, so `counted` is unchanged. Prior note: #964: RE-MEASURED 2026-07-24 (this PR) at 10 counted (was 12) — the #964 M9 SSR precision fix dropped 2 'SSR-only API misuse' FPs: a `window?.location` optional-chained read in a .tsx component (sign-in-methods-container.tsx — the author's absent-guard) and a `document.querySelector` read in the non-component `.ts` hook use-csrf-token.ts. Residual 10: 8 SSR-only + 2 accidental dynamic rendering. Prior note: Re-measured 2026-07-17: 2->12 — the 2 'Accidental dynamic rendering' plus 10 of #381's 'SSR-only API misuse' class in packages/features/auth sign-in/sign-up components." },
  ...
  },
  {
    ...
    modules: {
      ...
      M7: { counted: 15, total: 18, note: "#1306/#1353: RE-MEASURED 2026-07-28 at 15/18 (was 17/20) — two opposite movements that happen to net to -2, so read them separately. MEASURED by a controlled before/after `detect-static` on the identical pinned clone, one variable (this PR's detector diff). REMOVED (-4, all Await-in-loop N+1, all genuine FPs): packages/billing/src/scripts/{invoice-pending-prorations,subscription-data-sync,sync-payment-methods}.ts and packages/database/prisma/seed.ts — #230's sixth precision suppression (seed/build scripts), which PR #236 never implemented; `prisma/seed.ts` is the file #230 named by hand. ADDED (+2, Blocking sync I/O): packages/utils/src/encryption.ts:23 and :86 — the `crypto.pbkdf2Sync` rows #1353 recorded as SILENCED by #1344's reachability gate, now restored because resolveImport follows the `@rallly/utils` workspace specifier through that package's `exports` wildcard (./* -> ./src/*.ts). Prior note: #894: MEASURED 2026-07-24 — 9 await-in-loop N+1, 4 raw <img>, 1 nested-loop join, 1 whole-library lodash import, 1 sort-in-JSX, 1 oversized-committed-images roll-up (7 images, 9.3 MB); 3 hook-dep Info. NOTE for the Prisma tier: #761's unindexed-FK check contributes ZERO here — this target's schema.prisma carries no models (see schemaPath above), so the FK check has nothing to read. ghostfolio/documenso/inbox-zero are where that detector is baselined." },
      M8: {
  ...
  },
  {
    ...
    modules: {
      ...
      M7: { counted: 125, total: 256, note: "#1306/#1353: RE-MEASURED 2026-07-28 at 125/256 (was 127/258) — same controlled before/after as rallly, and the same two opposite movements. REMOVED (-4 Await-in-loop N+1, all FPs): packages/prisma/seed/{documents,initial-seed,medium-account-seed,organisations}.ts — #230's never-implemented sixth suppression. ADDED (+2 Blocking sync I/O): packages/lib/server-only/cert/cert-status.ts:21 (`fs.statSync`) and packages/signing/transports/google-cloud.ts:62 (`fs.writeFileSync`) — the two documenso rows #1353's table recorded as silenced, restored by workspace-specifier resolution. Prior note: #894: MEASURED 2026-07-24 — 127 counted with the corpus's largest Info tail (131 hook-dependency style notes), plus a 16-image/19.1 MB oversized-asset roll-up. The Info tail is itself worth pinning: #230 demoted that class rather than dropping it, and this is the target where the demotion carries the most weight." },
