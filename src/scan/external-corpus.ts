...
export const EXTERNAL_CORPUS: ExternalTarget[] = [
  ...
  },
  {
    ...
    modules: {
      ...
      M9: { counted: 15, total: 20, note: "#1353: RE-MEASURED 2026-07-28 at 15/20 (was 12/17) — +3, all real, all `Missing server-only guard`, from the same controlled before/after as the M7 row (pristine-main control at c597012 reprints 12/17). apps/web/src/lib/{session-config,maintenance,posthog}.ts each read a server secret from process.env with no `server-only` import, and each is now provably client-reachable: `apps/web/src/trpc/client/provider.tsx` ('use client') -> trpc/routers/index.ts -> routers/auth.ts -> lib/session.ts -> lib/session-config.ts, traced through the fixed graph. The cause is the `collectPathAliases` half of #1353 — it used to `break` at the shallowest tsconfig declaring `paths`, and rallly has no root tsconfig, so a SIBLING package's config won the tie and apps/web's own `@/*` -> `./src/*` was never read. Prior note: #1262/#1292: RE-MEASURED 2026-07-28 at 12/17 (was 12/15) — +2 total, NEITHER of them a detection change. MEASURED by a controlled before/after `detect-static` on the identical pinned clone, one variable (this PR's M9 diff): the before arm at 564bded reprints 12/16. Read the arithmetic carefully: the before arm on this PR's base commit already reads 12/16, not the 12/15 recorded here — a PRE-EXISTING undisclosed drift of +1 Info that this PR did not cause and is correcting in passing. Its cause is #1051's `M9 — Cross-user cache bleed — not assessed` row, which joined the three #903 not-assessed rows on this non-Supabase target and was never folded into this baseline. The +1 total is #1262's `M9-RETRY` scope row — a counted not-assessed row disclosing the three uncapped retry/fan-out sub-shapes the new AST detector does not reach, emitted on any target that has route/edge handlers at all. Info, so `counted` is unchanged. Prior note: #964: RE-MEASURED 2026-07-24 (this PR) at 12 counted / 15 total (3 Info not-assessed rows) — the #964 M9 SSR precision fix dropped 2 'SSR-only API misuse' FPs: a `document.createElement` read in the non-component `.ts` util apps/web/src/lib/image-processing.ts and a `navigator.userAgent` read in the non-component `.ts` hook packages/ui/src/hooks/use-platform.ts. Residual 12: 11 SSR-only + 1 accidental dynamic rendering (cookies() read). Prior note: #894: MEASURED 2026-07-24 — 13 SSR-only API misuse (11 window, 1 document, 1 navigator) + 1 'cookies() read in a page'." },
  ...
  //      adversarial positives, not a rushed widening on the day the FP was found.
  {
    ...
    modules: {
      ...
      "M5-slop": { counted: 550, total: 610, note: "#1276: MEASURED 2026-07-28 via detect-static (no install prereq) — 540 single-use helper, 32 narrating comment, 28 decorative emoji, 5 single-call wrapper, 4 else-after-return, 1 unused import. The single-use-helper density (540) is the shape the 2026-07-28 breadth sweep flagged as a rollup candidate across 15/15 repos, not a property of this target." },
