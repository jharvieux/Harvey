...
export const EXTERNAL_CORPUS: ExternalTarget[] = [
  ...
  },
  {
    ...
    modules: {
      ...
      "M5-knip": { counted: 3, total: 3, note: "#544: MEASURED 2026-07-18 (cloned pin + `npm install` + Harvey's per-workspace knip). The prior not-run reason — whole-repo knip dying inside eslint-config-next while loading apps/web/eslint.config.mjs — is STALE since #519 made knip run PER WORKSPACE. 3 counted (Low), all in tooling/eslint (apps.js/base.js/nextjs.js, flagged as unused FILES). HONEST CAVEAT: these are a per-workspace-scoping artifact — apps/web DOES consume them via @kit/eslint-config, but knip scanning tooling/eslint in isolation cannot see that cross-workspace reference. The result is a genuine PARTIAL: apps/web STILL fails knip with the exact eslint-config-next patch error the old reason named, and apps/e2e fails loading its playwright config — both disclosed in the M5-00 gap finding (#505); and the 7 `packages/**` workspaces aren't knip'd at all because quality-scan's workspace enumeration (shared discoverTargets) doesn't expand the `packages/**` double-star glob (#548). Recorded as a measured 3 rather than not-run so the scorecard reflects reality instead of a decayed excuse; re-verify if a dep bump makes apps/web's knip resolve or #548 lands." },
