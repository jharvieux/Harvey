...
export const EXTERNAL_CORPUS: ExternalTarget[] = [
  ...
  },
  {
    ...
    modules: {
      ...
      "M5-knip": { counted: 153, total: 154, note: "#894: MEASURED 2026-07-24 WITH `npm install` in the clone (67; the same run without deps read 69, recorded here so the delta is on the record). 17 unused files + 47 unused-export files + 1 Medium security-relevant. STILL A PARTIAL and the baseline says so: 4 of 16 scopes (apps/landing, apps/web, packages/database, packages/ui) ran in #810's reduced no-dependencies mode and are disclosed as the M5-98 Info row, because this is a pnpm workspace and `npm install` — what corpus-drift --install ran AT THE TIME — resolves only the 219 ROOT packages, leaving every workspace without node_modules. #1128: RE-MEASURED 2026-07-26 at 153/154 (was 67/68) — a precision fix from #1094/#1080 (commit 4469338, merged 2026-07-25): knip's six previously-dropped IssueRecords categories now surface, most visibly `unlisted`/`unresolved` given this target's partial-install shape. `pnpm corpus-drift --target rallly --install` reproduced 153 counted. #1268 note (2026-07-28): the drift job HAS since gained a pnpm install path (installTargetDeps now resolves via the target's own lockfile-implied package manager) — the 'Re-verify if the drift job ever gains a pnpm install path' condition above is now met. NOT re-measured in THIS PR (#1268 is scoped to M8): this 153/154 count is still the partial 4-of-16-scopes reading and very likely moves once M5-knip is re-run against a full pnpm install. Re-measurement tracked in #1404 — treat this number as stale until then." },
  ...
  // docs/design/carbon-scale-measurement.md; these are the baselines that completed.
  {
    ...
    modules: {
      ...
      "M5-knip": { counted: 2879, total: 2881, note: "#897: MEASURED 2026-07-24 WITHOUT the target's deps — and that is reproducible in CI, not a local shortcut: `npm install` FAILED on this target AT THE TIME with EUNSUPPORTEDPROTOCOL (`catalog:`, a pnpm-catalog dependency), which is exactly what corpus-drift's installTargetDeps swallowed. So knip ran in #810's reduced no-dependencies mode across the 36 workspaces and the M5-98 Info row is part of this baseline. A DRIFT baseline, not a dead-code claim. #1128: RE-MEASURED 2026-07-26 at 2879/2881 (was 2773/2775) — a precision fix from #1094/#1080 (commit 4469338, merged 2026-07-25): knip's six previously-dropped IssueRecords categories now surface, still in the same reduced no-deps mode. `pnpm corpus-drift --target carbon --install` reproduced 2879 counted. #1268 note (2026-07-28): `pnpm install` (not npm) resolves this target's install in full (MEASURED, 1m7s, no EUNSUPPORTEDPROTOCOL) — the EUNSUPPORTEDPROTOCOL blocker described above is no longer current. NOT re-measured in THIS PR (#1268 is scoped to M8): this 2879/2881 count is still the #810 reduced no-dependencies-mode reading and very likely moves once knip is re-run against a full pnpm install. Re-measurement tracked in #1404 — treat this number as stale until then." },
