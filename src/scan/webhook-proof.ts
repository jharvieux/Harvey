...
const STRIPE_SHAPE = runtimeShape(STRIPE_HMAC);
const COMPARE_SHAPE = runtimeShape(CONSTANT_TIME_COMPARE);
...
function provenVerifier(graph: SourceGraph, resolved: ResolvedFunction): boolean {
  if (runtimeShape(resolved.fn.getText(resolved.sf)) !== STRIPE_SHAPE) return false;
  const comparator = graph.resolve(resolved.path, "timingSafeEqual");
  if (!comparator || runtimeShape(comparator.fn.getText(comparator.sf)) !== COMPARE_SHAPE) return false;
