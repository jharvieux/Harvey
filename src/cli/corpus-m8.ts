...
try {
  assertKnownFlags(["--github-output", "--target", "--out", "--artifacts"], process.argv.slice(2).map((value) => redactSecrets(value)));
  ...
  function required(flag: string): string {
    ...
    if (!value) {
      console.error(redactSecrets(`${mode ?? "corpus-m8"}: ${flag} is required`));
      process.exit(2);
    }
  ...
  const detail = redactSecrets(error instanceof Error ? error.message : String(error));
  console.error(`M8 CORPUS FAILED: ${detail}`);
  process.exit(1);
}
