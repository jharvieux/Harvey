...
for (;;) {
  ...
  const page = (JSON.parse(r.stdout) as { data: { repository: { issues: { pageInfo: { hasNextPage: boolean; endCursor: string }; nodes: RelayCensusNode[] } } } }).data.repository.issues;
...
const { rows, duplicates } = collectRelayCensus(nodes);
...
if (args.includes("--json")) {
  console.log(JSON.stringify({ issues: nodes.length, comments: commentCount, linkedPrs: linkedPrCount, live, superseded, duplicates }, null, 2));
  process.exit(0);
}
...
console.log(`  duplicate-disposition edge: ${duplicates.length} criterion/criteria in the live population; disclosed separately because a duplicate has problems and no disposition`);
for (const r of live) console.log(`    LIVE        #${r.issue}.${r.index} — ${r.detail?.slice(0, 90)}`);
for (const r of superseded) {
  console.log(
    `    SUPERSEDED  #${r.issue}.${r.index} — retired by ${r.retiredBy?.disposition} at ${r.retiredBy?.venue},` +
      ` line ${r.retiredBy?.line}; retired relay was at ${r.retired?.venue}, line ${r.retired?.line}`,
  );
}
for (const duplicate of duplicates) console.log(`    DUPLICATE   #${duplicate.issue}.${duplicate.index} — ${duplicate.problem}`);
