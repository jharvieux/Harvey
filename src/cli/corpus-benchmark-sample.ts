...
try {
  ...
  const priorScorecardPolicies = byKind("benchmark-prior-scorecard-policy").map((path) => parseJson<CorpusBenchmarkPriorScorecardPolicy>(join(evidenceDir, path)));
