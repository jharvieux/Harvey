...
export const CLAIM_BASELINE: Record<string, string[]> = {
  ...
  "CLAUDE.md": [
    ...
    "- **\"Not required\" is not \"not important\" (2026-07-30).** A non-required check that FAILS, or that could not run, still has to be read. `corpus-drift` is not a required context; it died at setup for days, reported nothing distinguishable from a clean run, and a PR merged on three green required checks while the gate measuring its blast radius had never executed — 653 findings silently suppressed and a live regression on `main`. Before merging on green, ask which gates actually EXECUTED, not which are green: a check that cannot run is not a check that passed.",
