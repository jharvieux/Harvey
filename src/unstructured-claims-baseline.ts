...
export const CLAIM_BASELINE: Record<string, string[]> = {
  ...
  "CLAUDE.md": [
    ...
    "- **Base branch: `main`.** It is protected: every change lands through a PR and all six currently required context names must report success: `verify`; `regenerate dry-run findings + diff committed artifact`; `plant → deliverable, and the produced/delivered arithmetic`; `clone pinned commits + score baselines`; `every closed issue's acceptance criteria are accounted for`; and `a recorded bound reaches the finding it bounds`. GitHub cannot make a required context conditional, so every name reports on every PR. Substantive work remains relevance-routed: when a diff cannot affect a context's owned inputs, that workflow emits a declared no-op success naming the reason and liveness receipt rather than omitting the context or implying validation ran. Re-measure the names and settings immediately before merge (`gh api repos/jharvieux/Harvey/branches/main/protection/required_status_checks`); a stored count is a dated claim, and CI contexts are not the same population as gates. In particular, `acceptance on close` runs on `issues: [closed]` and remains structurally ineligible for branch protection; the PR context `every closed issue's acceptance criteria are accounted for` is a separate close-set check. Then squash-merge and delete the branch. **Budget for it, but re-measure rather than quoting this history.** #1586 sharded corpus drift across three runners on `pull_request`/`merge_group` (one on `schedule`, because the clone cache can only be saved by a run holding all pinned clones). Measured 2026-07-31: single-job n=3 averaged 22m04s; three-shard PR runs n=5 averaged 10m59s at the slowest shard and 11m22s from run creation to required-context completion, while raising total runner minutes about 22.5%. A PR is still not mergeable when the other five contexts are green.",
