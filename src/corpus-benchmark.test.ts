...
function sample(overrides: Partial<CorpusBenchmarkSample> = {}): CorpusBenchmarkSample {
  ...
  return {
    ...
    actualRunner: { jobs: [{ id: 1, name: "GitHub Actions 1" }, { id: 2, name: "GitHub Actions 2" }, { id: 3, name: "GitHub Actions 3" }], group: "GitHub Actions", labels: ["ubuntu-latest"], image: "ubuntu24", cpuLimit: 4, memoryLimitBytes: 16_000 },
    profile,
    design: overrides.design ?? "serial",
    concurrency: overrides.concurrency ?? 1,
    effectiveConcurrency: overrides.effectiveConcurrency ?? overrides.concurrency ?? 1,
    shardCount: overrides.shardCount ?? 3,
    ...
    raw: { workflowUrl: "https://example.test/run/1", jobIds: Array.from({ length: overrides.shardCount ?? 3 }, (_, index) => index + 1), artifactDigests: [digest("8")], toolVersions: { node: "v24" }, targetCommits: { alpha: "b".repeat(40), beta: "c".repeat(40) } },
    ...overrides,
  };
}

function triplet(overrides: Partial<CorpusBenchmarkSample>): CorpusBenchmarkSample[] {
  return [1, 2, 3].map((repeat) => sample({ ...overrides, repeat, runId: Number(`${overrides.shardCount ?? 3}${overrides.concurrency ?? 1}${repeat}`) }));
}
...
describe("#1873/#1868/#1875 corpus benchmark evaluator", () => {
  ...
  it("keeps concurrency and shard selection inside the standard PR runner cohort", () => {
    const matrix = [
      ...qualifyingMatrix(),
      ...triplet({ profile: "cold", design: "target-workers", concurrency: 3, shardCount: 3, criticalPathMs: 790, aggregateRunnerMs: 2_100 }),
      ...triplet({ profile: "warm", design: "target-workers", concurrency: 3, shardCount: 3, criticalPathMs: 790, aggregateRunnerMs: 2_100 }),
    ];
  ...
  it("fails loud on OOM/retry, missing repeats, and mixed head/population evidence", () => {
    const oom = qualifyingMatrix();
    oom[0]!.oomCount = 1;
    expect(() => evaluateCorpusBenchmark(oom)).toThrow(/OOM\/retry/);

    expect(() => evaluateCorpusBenchmark(qualifyingMatrix().filter((row) => !(row.design === "serial" && row.profile === "cold" && row.repeat === 3)))).toThrow(/three distinct real repeats/);

    const mixed = qualifyingMatrix();
    mixed[0]!.headSha = "f".repeat(40);
    expect(() => evaluateCorpusBenchmark(mixed)).toThrow(/mixes head identities/);
    const population = qualifyingMatrix();
    population[0]!.population.findingDigest = digest("9");
    expect(() => evaluateCorpusBenchmark(population)).toThrow();

    const fakeSplit = qualifyingMatrix();
    fakeSplit[0] = { ...fakeSplit[0]!, design: "split-carbon", effectiveConcurrency: 1 };
    expect(() => evaluateCorpusBenchmark(fakeSplit)).toThrow(/did not execute a separate lane/);
  });
});
