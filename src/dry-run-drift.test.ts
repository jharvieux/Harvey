...
describe("dry-run PR relevance", () => {
  ...
  it("preserves a proved unrelated documentation no-op", () => {
    const result = runShippingFilter("export {};\n", "docs/input.md");
  ...
  it("executes the shipping selector against literal, computed, indirect and child-process data inputs", () => {
    ...
    for (const source of sources) {
      const result = runShippingFilter(source, "docs/input.md", true);
  ...
  it("tracks a literal directory read across a newly added documentation member", () => {
    const result = runShippingFilter('import { readdirSync } from "node:fs"; console.log(readdirSync("docs").sort().join(","));', "docs/new.md", true);
...
function runShippingFilter(source: string, changed: string, executeProducer = false): { output: string; before?: string; after?: string } {
  ...
  const producer = (): string => execFileSync(process.execPath, ["--import", loader, join(repo, "src/cli/dry-run.ts")], {
    cwd: repo, encoding: "utf8", env: { ...process.env, PRODUCER: join(repo, "tools/producer.mjs") },
  });
  const before = executeProducer ? producer() : undefined;
  writeFileSync(join(repo, changed), changed.endsWith(".ts") ? "export const changed = true;\n" : "after\n");
  git(["add", "."]); git(["commit", "-qm", "change"]);
  const after = executeProducer ? producer() : undefined;
  ...
  const run = spawnSync("bash", ["-e", "-o", "pipefail", "-c", script.replaceAll("${{ github.event_name }}", "pull_request").replaceAll("${{ github.event.pull_request.base.sha }}", base)], {
