...
describe("bound audit replay", () => {
  ...
  it("rejects a substituted derived result and missing command catalogs from real process evidence", async () => {
    for (const defect of ["result", "missing-catalog", "empty-catalog"] as const) {
      const f = fixture();
      const report = join(f.root, "M1-command.json");
      const actual = await probeExec(process.execPath, ["-e", "require('node:fs').writeFileSync(process.argv[1], JSON.stringify({ observed: 'run-a' }))", report], { receipt: { artifacts: [{ role: "report", path: report }] } });
  ...
  it("checks derived-result ownership again during replay even when bundle checksums are consistent", async () => {
    const f = fixture();
    const report = join(f.root, "M1-command.json");
    const actual = await probeExec(process.execPath, ["-e", "require('node:fs').writeFileSync(process.argv[1], 'actual report')", report], { receipt: { artifacts: [{ role: "report", path: report }] } });
    installOwningRun(f, report, [actual.receipt]);
    f.write();
    ...
    await expect(replayAuditBundle(f.bundle, f.target, { now: f.now })).rejects.toThrow(/owning-run derived report/);
  });

  it("retains explicit in-process module evidence without inventing child command attempts", async () => {
    const f = fixture();
    ...
    f.write();
    await expect(replayAuditBundle(f.bundle, f.target, { now: f.now })).resolves.toBeDefined();
  });

  it("distinguishes byte-identical reports by their original artifact identity", async () => {
    const f = fixture();
    const reports = [join(f.root, "first.json"), join(f.root, "second.json")];
    const receipts = (await Promise.all(reports.map((report) => probeExec(process.execPath, ["-e", "require('node:fs').writeFileSync(process.argv[1], '[]')", report], { receipt: { artifacts: [{ role: "report", path: report }] } })))).map((result) => result.receipt);
  ...
  it.each(["executionReceipt", "nativeComparison"])("validates nested %s outputs against the original command", async (kind) => {
    const f = fixture();
    const nestedReport = join(f.root, "native.json");
    const nested = (await probeExec(process.execPath, ["-e", "require('node:fs').writeFileSync(process.argv[1], 'actual')", nestedReport], { receipt: { artifacts: [{ role: "report", path: nestedReport }] } })).receipt;
  ...
  it("binds an accepted report to one exact invocation and rejects regeneration, mixed runs and duplicate invocation IDs", async () => {
    ...
    accepted.write();
    await expect(replayAuditBundle(accepted.bundle, accepted.target, { now: accepted.now })).resolves.toBeDefined();
  ...
  it("conserves fresh producer findings and explicit scopes without invoking any runner", async () => {
    const f = fixture();
    const env = { connected: false, dynamic: false, llm: false };
    const fresh = await runAudit(f.passes.map((pass): ModuleRunner => ({ module: pass.scope.module, producers: [], typed: true, run: () => pass.result })), { targetDir: f.target, env, exists: () => true, exec: async () => { throw new Error("fixture has no external capability"); } });
    f.write();
    ...
    const replay = await replayAuditBundle(f.bundle, f.target, { now: f.now });
  ...
  it.each(["target", "configuration", "raw", "missing", "manifest", "future", "stale"])("rejects %s evidence before delivery", async (kind) => {
    ...
    await expect(replayAuditBundle(f.bundle, f.target, { now, ...(kind === "configuration" ? { effectiveConfig: { mutate: ["unrelated.ts"] } } : {}) })).rejects.toThrow(/mismatch|Tampered|tampered|Missing|Stale/);
  });

  it("keeps missing modules and application routes partial after a PostgREST-only pass", async () => {
    const f = fixture();
  ...
  it.each(["", "\nM3 REDUCED TIER", "\nM3 UNRANKED"])("retains recorded M3 trend evidence when the base hotspot probe also succeeds (%s)", async (banner) => {
    const planted = finding("M3-TREND-00", "M3");
    const result = await AUDIT_RUNNERS.find((runner) => runner.module === "M3")!.run({ targetDir: "/fixture", env: { connected: false, dynamic: false, llm: false }, captureDir: "/capture", artifactsDir: "/passes", exists: () => true, exec: async () => ({ ok: true, output: `M3 hotspot table — /fixture (4 rows, worst first)${banner}` }), readArtifact: (path) => path.includes(".pass.") ? { module: "M3", target: "/fixture", pass: "trend", generatedAt: new Date().toISOString(), findings: [planted] } : { findings: [finding("M3-BASE", "M3")], topK: [] } });
  ...
  it.each([[75.1, 29.4], [29.4, 75.1]])("keeps native M8 score %s over older recorded score %s, including its own scope", async (nativeScore, recordedScore) => {
    const now = Date.now();
    ...
    const result = await AUDIT_RUNNERS.find((runner) => runner.module === "M8")!.run({ targetDir: "/fixture", env: { connected: false, dynamic: false, llm: false }, captureDir: "/capture", artifactsDir: "/passes", now, exists: () => true, exec: async (_command, args) => ({ ok: true, output: args.includes("mutation-scan") ? JSON.stringify(artifact) : "loaded 4 source files (4 product source, 0 config, 0 test/story) from /fixture" }), readFindings: () => [], readArtifact: (path) => path.includes(".pass.") ? { module: "M8", target: "/fixture", pass: "mutation", generatedAt: new Date(now - 60_000).toISOString(), findings: [], testQuality: quality(recordedScore) } : artifact });
  ...
  it.each([false, true])("uses the newest recorded M8 table when the native invocation has no measurement (reverse order=%s)", async (reverse) => {
    const now = Date.now();
    ...
    const result = await AUDIT_RUNNERS.find((runner) => runner.module === "M8")!.run({ targetDir: "/fixture", env: { connected: false, dynamic: false, llm: false }, captureDir: "/capture", artifactsDir: "/passes", now, exists: () => true, exec: async (_command, args) => ({ ok: !args.includes("mutation-scan"), output: args.includes("mutation-scan") ? "blocked" : "loaded 4 source files (4 product source, 0 config, 0 test/story) from /fixture" }), readFindings: () => [], readArtifact: () => reverse ? { ...old, priorPasses: [latest] } : { ...latest, priorPasses: [old] } });
    const measured = (Array.isArray(result) ? result[0] : result) as Examined;
    expect(measured.testQuality?.mutationScore).toBe(29.4);
    expect(measured.reason).toContain("mutation tier blocked");
  });

  it("keeps advisor-only M7 findings without inventing Lighthouse measurements or absent credentials", async () => {
    const result = await AUDIT_RUNNERS.find((runner) => runner.module === "M7")!.run({ targetDir: "/fixture", env: { connected: false, dynamic: false, llm: false }, captureDir: "/capture", artifactsDir: "/passes", exists: () => true, exec: async () => ({ ok: true, output: "loaded 4 source files (4 product source, 0 config, 0 test/story) from /fixture" }), readFindings: () => [], readArtifact: () => ({ module: "M7", target: "/fixture", pass: "advisors", generatedAt: new Date().toISOString(), summary: "Authenticated advisors completed", findings: [finding("M7-ADVISOR", "M7")] }) }) as Examined;
  ...
  it("keeps the full connected permission failure and executable repair instruction", async () => {
    const detail = `${"response preamble ".repeat(20)}HTTP 403: project.read permission denied`;
    const result = await AUDIT_RUNNERS.find((runner) => runner.module === "M7")!.run({ targetDir: "/fixture", env: { connected: true, dynamic: false, llm: false }, supabaseRef: "project", exists: () => true, exec: async (_command, args) => args.includes("perf-scan") ? { ok: false, output: detail } : { ok: true, output: "loaded 4 source files (4 product source, 0 config, 0 test/story) from /fixture" } });
