...
export const CORPUS_BENCHMARK_PRIOR_SCORECARD_POLICY: CorpusBenchmarkPriorScorecardPolicy = {
  schema: 1,
  mode: "disabled-for-benchmark",
  reason: "prior-scorecard-is-diagnostic-only",
};
...
export interface CorpusBenchmarkSample {
  ...
  raw: {
    ...
    priorScorecardPolicy: CorpusBenchmarkPriorScorecardPolicy;
...
interface CorpusBenchmarkDecision {
  ...
  provenance: { headSha: string; benchmarkSeed: string; benchmarkSeedRunId: number; benchmarkSeedRunAttempt: number; priorScorecardPolicy: CorpusBenchmarkPriorScorecardPolicy; targetDigest: string; evidenceDigest: string; sampleRunIds: number[]; sampleDigest: string };
  concurrency: {
...
function cohortIdentity(sample: CorpusBenchmarkSample) {
  ...
  return {
    ...
    priorScorecardPolicy: sample.raw.priorScorecardPolicy,
...
function assertCohortRows(rows: readonly CorpusBenchmarkSample[], label: string): CorpusBenchmarkSample[] {
  ...
  if (identities.size !== 1) throw new Error(`${label}: cohort mixes actual runner, prior-scorecard policy, tool/runtime, target/evidence, role, profile, design, concurrency, shard, or requested-runner identities`);
...
function assertSampleEnvelope(sample: CorpusBenchmarkSample): void {
  ...
  const priorPolicyArtifacts = artifacts.filter((artifact) => artifact.family === "evidence/benchmark-prior-scorecard-policy");
  if (priorPolicyArtifacts.length !== sample.shardCount) {
    throw new Error(`${label}: retained prior-scorecard policy artifact population is incomplete`);
  }
  if (new Set(priorPolicyArtifacts.map((artifact) => artifact.sha256)).size !== 1) {
    throw new Error(`${label}: retained prior-scorecard policy artifacts do not contain identical bytes`);
  }
...
function assertSample(sample: CorpusBenchmarkSample): void {
  ...
  if (stable(sample.raw.priorScorecardPolicy) !== stable(CORPUS_BENCHMARK_PRIOR_SCORECARD_POLICY)) {
    throw new Error(`run ${sample.runId}: prior-scorecard policy is missing, mixed, or not disabled for benchmark execution`);
  }
...
function assertMatchedPopulation(samples: readonly CorpusBenchmarkSample[]): { headSha: string; benchmarkSeed: string; benchmarkSeedRunId: number; benchmarkSeedRunAttempt: number; priorScorecardPolicy: CorpusBenchmarkPriorScorecardPolicy; targetDigest: string; evidenceDigest: string } {
  ...
  if (evidence.size !== 1) throw new Error("benchmark mixes head, seed, prior-scorecard policy, target/evidence, tool/runtime, target-commit, or assertion identities");
  ...
  return {
    ...
    priorScorecardPolicy: first.raw.priorScorecardPolicy,
