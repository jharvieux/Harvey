...
function run(name, directory) {
  ...
  const child=spawnSync(process.execPath,['--import','tsx','src/cli/mutation-scan.ts',directory,'--concurrency','1','--out',out],{cwd:engine,encoding:'utf8',maxBuffer:64*1024*1024});
  ...
  return {path:out,artifact:JSON.parse(readFileSync(out,'utf8'))};
}
const atc=run('atc',fixture('atc-source',['apps/main','apps/rag']));
...
const mixedSource=fixture('mixed-source',['apps/main','apps/none','apps/unsupported','apps/dry','apps/missing']);
...
const overrideChild=spawnSync(process.execPath,['--import','tsx','src/cli/mutation-scan.ts',overrideSource,'--plan','--config',override,'--out',overrideOut],{cwd:engine,encoding:'utf8'});
