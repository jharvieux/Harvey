...
| 12 | ANON-PRIVILEGED-RPC | Critical | `supabase/migrations/20260710000002_dynamic_probes.sql` (`issue_refund`) | `issue_refund` is SECURITY DEFINER, EXECUTE granted to `anon`, no `auth.uid()` check | `POST /rest/v1/rpc/issue_refund` with only the anon key executes a refund; `promote_admin` has EXECUTE revoked from `public, anon` (negative — the `public` half is required, #1671: Postgres' default PUBLIC grant is independent of Supabase's per-role grant, so a `from anon`-only revoke left `has_function_privilege('anon', …, 'EXECUTE')` TRUE). |
...
| M4-P-CLONE-SELF | `dup/self-file-shipping.ts` (in-file, `summariseDomesticShipment` ↔ `summariseInternationalShipment`) | a 16-line accumulate-and-format block copy-pasted TWICE INSIDE ONE FILE; jscpd clone cluster with both refs on one path. #232 excluded the whole self-file band as inert repeated DATA, #1080 measured that false and #1095 (operator ruling 2026-07-26) restored it — `jscpdToFindings` now scores it like any other clone (Low, 15–49 lines) with in-file fix text and a collapsed `path:a-b ↔ :c-d` location, instead of dropping it into the `M4-SELF-00` count | high |
| M4-P-DIVERGED-SELF | `dup/auth/workspace-guard.ts` (in-file, `assertWorkspaceMember` ↔ `assertProjectMember`) | two copies of one membership guard 12 lines apart that have DIVERGED on the ownership column (`row.owner_id` vs `row.id`) with drifted error strings. No contiguous exact span reaches jscpd's 5-line/50-token floor, and before #1095 BOTH near-miss passes `continue`d on `a.path === b.path` — so nothing in M4 could see this pair. The #360 pass now emits an `M4-DIV-*` review finding (High) naming the drift | review |
...
| M4-N-DIV-DISTINCT | `dup/auth/api-key-check.ts` | an independently-written guard in the same auth path — same domain, different structure. The #360 pass must not pair it with the require-tenant guards (similarity-floor FP guard). Note the `session-check-*` pair is also a #360 negative by construction: a CONSISTENT rename (Type-2) is faithful duplication jscpd already reports, not drift — asserted in `src/diverged-clones.test.ts`. The one FP shape only same-file comparison can produce — a NESTED function, a token subsequence of its parent that LCS scores as near-identity — is rejected on line ranges by `compare()`'s `isNested` (#1095) and held by a unit test rather than a corpus fixture: it is a property of the comparison loop, not of a target's code. |
