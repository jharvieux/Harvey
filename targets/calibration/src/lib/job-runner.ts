...
// `params` / `searchParams`.
//
// This is the shape #1240's RSC taint source got wrong. That source matches `params.<field>` by
// NAME (semgrep OSS cannot match a destructured object parameter, which is what an RSC page
// actually receives), and its only guard excluded a name bound by ASSIGNMENT — a function parameter
// is never assigned, so every one of these lines became request-tainted. MEASURED 2026-07-27: this
// file's twin with the parameter renamed to `opts` produced 3 findings; this one produced 6,
// including `harvey-command-injection` at Critical / precisionTier "high", i.e. GRADED.
//
