// dup/auth/session-check-action.ts — session/tenant validation as used by server actions.
...
export function validateActionSession(session: ActionSession | null, requestedTenantId: string): string[] {
  const problems: string[] = [];

  // PLANTED CLONE (M4-P-CLONE-SEC): identical session/tenant validation block, copy-pasted
  // from session-check-api.ts instead of being extracted into a shared guard helper.
