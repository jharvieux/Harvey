...
**All four operator-assigned issues are DONE and merged:** #1664+#1746 (PR #1749), #1696 (PR #1751), #1752 (PR #1761), #1710 (PR #1772), plus #1753 (PR #1766), drill #1750 executed (#1348 reopened), #1767 closed. Every PR got an independent acceptance verifier before merge; every close survived the gate. Main green at session end.

- **The #1710 finding is the headline: semgrep 1.164.0's default threaded mode silently loses 3-5% of findings on large repos** — strict-subset loss (never extra rows), mostly traceless, reproduced independently on two machines. `runSemgrep` now pins `--x-parmap --timeout 0` (fallback `-j 1 --timeout 0`), byte-stable 7/7 and 9/9 across both machines, CHEAPER than the default on heavy repos, first CI exercise green against committed baselines. Every historical large-repo scan under the old default under-reported. **The pin is version-specific: #1771 re-earns it on the next semgrep bump** and now carries the verifier's second-machine data (recall bound is per-machine: 5 rows vs 2, second fragile file).
- **The #1749 swallow pattern generalises.** Sweep of every external-binary call site: #1754 (trufflehog/gitleaks NDJSON — worst, engagement path), #1755 (splinter psql, paid connected tier), #1756 (secbench bypasses execSemgrep), #1757 (3 lower-stakes sites, one already fixed inline by PR #1761). Cleared as NOT the shape: Stryker (report file is ground truth), jscpd (gap row), pentest stand-ups. **#1754-#1756 filed, NOT dispatched — operator has not asked yet.**
- **The reopen-always close gate (PR #1751) fired correctly five times in one evening** — the drill, twice on defective supervisor closes of #1767, and its grammar rejected two more attempts for rules learned only by collision. All recorded on #1769 (now four residuals): ruled-unmet relays have no honest completion form; the #1753 census reads venues the gate does not; inline-backtick refs are harvested as split remainders (the gate's own output is unquotable in a split); a standalone `remainder:` line is liveness-checked but NOT credited to the split — the ref must be inline.
- **Main went red twice, both fixed within the hour:** the validate-reasons fixture (other session, PR #1748) and THIS session's SESSION.md prose quoting a reserved ratchet word (PR #1762) — docs-only PRs skip verify, so the one gate that censuses .md never ran on the PR that only changed .md. Systemic fix proposed in **#1763** (~2s ratchet test on docs-only PRs + mention-vs-use quoting). Supervisor lesson recorded: run `src/recorded-reasons.test.ts` against edited .md BEFORE pushing a docs-only PR.
- **Relay completions (#1767, closed):** of the census's 3 stale relays, one was completable (#1424.4 — the supersession rule's first live use), one was a census false positive (#1345.1 — valid `met` already in the linked PR), one is structurally stuck (#1461.3 — ruled stays-unmet, remainder closed). #1345's own remainder #1349 is CLOSED — a decayed deferral OUTSIDE the labelled population.

### Parked on the operator (carry forward)

1. **The nine silently-closed issues** (#1347 #1342 #1341 #1308 #1302 #1301 #1297 #1296 #1285): dispatch the gate against them (reopens all nine — honest state) + repair dispositions. The #1345/#1349 finding suggests widening to a general read-only decayed-deferral sweep first.
2. **#1754-#1756**: fixes filed, awaiting dispatch decision. #1754 (secrets scan) is the consequential one.
3. **Ratchet mention-vs-use** (#1763 + the close-gate's identical gap on #1769): supervisor recommended keeping .md censused and adding quoting awareness; awaiting ruling.
