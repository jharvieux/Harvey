...
| **jscpd 60s ceiling** | **Still open — the one live question of the six.** A free scan can spend up to a minute on duplication before timing out into a disclosed not-assessed row, while the free scan is marketed as instant. Three answers: keep 60s; lower it to a number a prospect will wait; or re-measure jscpd across the pinned corpus and pick the ceiling from the distribution. The third needs no product ruling, only a yes to spending the run — it is a performance bound, not a product decision. |
...
| **jscpd 60s ceiling** (2026-07-30) | Third question from 2026-07-28, still unanswered; #1468 treated it as unchanged. A performance bound, so it can be re-measured rather than ruled — but the free scan is sold as instant. Want a number, or an explicit "re-measure, do not rule"? |
