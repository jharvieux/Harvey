...
**But Harvey's own AST layer is not the semgrep engine, and it does follow imports** (corrected 2026-07-31, #1267). `resolveImport`/`buildImportGraph` (`src/detectors/app-router.ts`, #380) resolve tsconfig path aliases and workspace packages, and nine production modules already build on them — the M9 server-only and client-reachability passes, the M7 entry-closure passes, `src/scan/ssrf-wrapper.ts` (route → imported helper → `fetch()`), and since #1267 `src/scan/bola-cross-file.ts` (authenticated route → imported repository → an ownership-scoped query on a service-role client). So a specific cross-file chain, named and shaped, is a detector we can write, and two of them ship. What stays paid is the OPEN-ENDED version: deciding whether *any* path through an arbitrary codebase authorizes a request, which needs intent, not reachability. Every such detector ships at **review** tier, because an AST proves the chain, never that no gate exists in a module it did not read.
