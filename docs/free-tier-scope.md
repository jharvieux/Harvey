...
- **Permission-matrix coverage — the INTENDED-AUTHORITY half only (#1370 split this, 2026-07-30; narrowed 2026-07-31).** Which cells the roles × resources matrix *ought* to contain is a question about intended authority, not about code shape, and stays with the semantic pass and the M2 pen-test. Which cells *exist* is code shape, and the free tier reads them: `checkMigrationRlsCommandCoverage` (`src/scan/supabase-static.ts`) builds table × command × role off the live policy set at the end of migration history — `parseLivePolicies` has carried both axes since #937. **What it REPORTS is one cell of that matrix, not every empty one** (`SB-RLS-CMDGAP-<table>`, review tier, one row per table, never graded): a table that grants the client INSERT/UPDATE/DELETE and NO SELECT policy — rows the client can write but has no policy to read back. An uncovered WRITE is deliberately NOT reported, and the reason is measured rather than assumed: the first version reported every empty cell and failed mvp-boilerplate's #227 don't-cry-wolf control on five sound tables (`corpus-drift` run 30634899914), because a read-policed table whose writes are server-side is the dominant Supabase arrangement and no migration text separates it from a hole. Each row states that bound in its own evidence and counts the write-only gaps it spared. Note what the row does and does not claim: a missing policy is **fail-closed** (Postgres denies the command), so it is not "this table is exposed" — it is "if the client can write these rows, something else reads them, and that path's tenant scoping is application code this RLS pass does not read." The sentence this bullet replaced declined the whole class as intended-authority-only, untested, in the same commit that retired the advertised capability.
