...
2. `npm install --ignore-scripts` in the target (confirmed default, see M5 row) — unlocks M5 (knip), M8 stub-check/Stryker candidacy, M7 Lighthouse/bundle builds. Not yet re-measured for M7 Lighthouse/bundle (a build step can depend on a skipped postinstall codegen in ways M5/M8 did not on the one target measured so far) — treat that combination as still open.
