...
2. `npm install --ignore-scripts` in the target (confirmed default, see M5 row) — unlocks M5 (knip), M8 stub-check/Stryker candidacy, M7 Lighthouse/bundle builds. **Measured for the M7 build 2026-07-18 (#540)** against `vandyand/saas-security-teardown`: `next build` succeeds identically under `--ignore-scripts` and a normal install (same route table, same 12 static pages). **But that target has no postinstall codegen** (deps: `@supabase/*`, `next`, `react`, `ws` — none generate code), so it does NOT exercise the case #540 actually warns about: a target whose `next build` imports a generated client (`prisma generate` / GraphQL codegen) that `--ignore-scripts` skips. On such a target the build can still fail with `--ignore-scripts` unless a targeted `prisma generate` runs first — **that codegen-dependent-build case remains unmeasured** (needs a buildable Prisma/codegen Next app; `boxyhq/saas-starter-kit` from #531 is a candidate but its full build needs DB env). Carry as open.
