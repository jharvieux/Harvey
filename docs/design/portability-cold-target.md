...
**Amended 2026-07-26 (#1042):** the rows below name a pass artifact only for M1/M2/M3/M6 (the four
whose status it can decide) and M7. **All ten modules now consume one** — M4/M5/M8/M9/M10 merge a
recorded pass's findings and NAME it on their row, upgrading `requires-live-run` → `partial`,
without ever claiming `ran`. So a recorded pass is a degrade path for every module in this table,
not only the ones whose "Ledger status" column mentions it.
...
| **M4 duplication** (`quality-scan`) | Nothing from the target beyond source — jscpd + diverged-clone pass run from Harvey's own node_modules | None needed | Known hazard: whole-monorepo runs can hang → run per-workspace + timeout→partial (#505, **CLOSED** — no longer an open provisioning caveat) | `ran` (jscpd genuinely executes on exit 0, #350 — and since #1065/#1088 the zero-files guard behind that `ran` can actually reach its trigger value, which for nine months it could not) |
...
| **M7 Lighthouse** (`lighthouse-scan`) | Buildable+servable target: installed deps, working `npm run build`/`start` (target/provisioned); **a system Chrome** (operator — the Playwright "Chrome for Testing" build NO_FCPs, see below) | Harvey installs deps, builds, serves locally (#387) — or `--url` at an already-running instance | Build/serve/browser failure → **M7L-00 disclosure finding**, exit 0. **Validated end-to-end 2026-07-18 (#488)** against `vandyand/saas-security-teardown`: install → `next build` → `next start` → Chrome → Lighthouse → real `M7L-01` LCP finding (`/security` 3.1s, `/dashboard` 2.6s). Two live bugs fixed: (a) a NO_FCP/errored run silently reported "0 findings" (false clean) — now surfaced as M7L-00 via `lighthouseRunErrorReason`; (b) the default browser (Playwright chromium) NO_FCPs — the CLI now prefers a system Chrome, Playwright is fallback-only. Details: `docs/design/m7-lighthouse-validation.md`. **Orchestrator gap CLOSED (#527/#1042):** `run-audit`'s M7 probe still does not *invoke* Lighthouse in-process, but it no longer ignores the tier — an M7 row with no evidence carries the `M7_LIGHTHOUSE_NOT_RUN` reason naming the unmeasured CWV tier, and a fresh `M7.pass.json` REPLACES that claim (rather than appending to it), merging the pass's findings into the deliverable. A present-but-rejected artifact is named on the row too. Before #1042 a recorded M7 pass was accepted by `record-pass` and then silently dropped | reason names the missing CWV tier; a fresh pass artifact replaces it and merges its findings (never `ran` on its own) |
