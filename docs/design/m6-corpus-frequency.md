...
## Source population and local measurements (#2105, #2103)

The frequency and admission commands share the product-source predicate beside
`SOURCE_FILE` in `src/detectors/load-sources.ts`: `.js`, `.jsx`, `.ts`, `.tsx`,
`.mjs`, `.cjs`, `.mts`, and `.cts`, with test/story/fixture paths excluded by
`NON_PRODUCT`. Frequency and density measurements also use the loader's generated-content
exclusions. Commit admission examines changed paths in Git history; it does not inspect
historical file contents for generated-code markers. Its denominator is product-touching
commits, while frequency reports selected source files and product LOC.

Use `pnpm handrolled-frequency --local <source-tree>` to measure a local source tree,
or `pnpm genai-admission-census --local <git-source-tree>` to inspect its local commit
history. These modes do not clone the external corpus or remove the supplied tree.
The admission command rejects combining `--local` with `--density`. The historical
measurements below retain their original population and have not been regenerated by
adding these local modes.
