...
2. **Next detector batch** — the YES column, in corpus-measured order (see
   ...
   are unranked), fixtures first. The per-library dep-gate the entries need is DONE (PR #409:
   `depGatePresent`, fails closed) — the dep-gated entries are unblocked on that front.
