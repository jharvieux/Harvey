...
| 18 | HTML-entity escaping via replace chain (`&amp;`, `&lt;`…) | framework escaping (React JSX auto-escapes) | **MAYBE + M1 check** | Replace-chain literal is crisp, but when the output feeds `dangerouslySetInnerHTML` this is M1 XSS territory (b4) — settle the boundary before building. |
...
| 47 | Polling via `setInterval` + fetch | SWR/React Query `refreshInterval` — dep-gate | **MAYBE + M7 check** | M7's `detectClientFetchEffect` (shipped 2026-07-16) covers the useEffect-fetch shape — check whether interval-polling is already inside its net before building anything under M6. |
...
| # | Shape | Standard replacement | Verdict | Reason / negative class |
|---|---|---|---|---|
| 65 | Manual `<title>`/`<meta>` JSX in App Router pages | Metadata API (`export const metadata`) | **MAYBE + M9 check** | Crisp shape, but App Router conventions are M9's charter — settle which module owns it before building. |
...
| 67 | Middleware `pathname.startsWith` ladder | `config.matcher` | **MAYBE + M1 check** | The security half (matcher excluding `/api`) is already corpus entry `P-MW-MATCHER-EXCLUDES-API`; the maintainability half must not double-count it. |
...
| 72 | Pages Router idioms pasted into `app/` (`_app`/`_document` patterns) | App Router conventions | **MAYBE + M9 check** | M9 already owns Pages-vs-App detection pieces (#231's gating); check before building. |
...
| 74 | Fetch-all then `.data.length` as a count | `count: "exact", head: true` | **MAYBE + M7 check** | Fetching all rows to count them is also a perf finding — the ATC M7 calibration already treats `head:true` count-only selects; check M7's net first. |
...
| 102 | Module-level Map cache with `Date.now()` TTL | React `cache()` / `unstable_cache` / Redis | **MAYBE + boundary check** | The rate-limiter variant is anti-pattern #19 (security-adjacent, guarded in ATC); the cache variant is maintainability plus a per-instance serverless caveat. Settle the boundary first. |
...
| MAYBE — deferred, with the specific blocker named | 34 | 5, 6, 12, 15, 18, 19, 22, 31, 32, 34, 35, 39, 40, 43, 47, 58, 59, 65, 66, 67, 69, 72, 73, 74, 75, 82, 83, 90, 92, 96, 99, 100, 101, 102 |
| NO — stays LLM-tier (paid packet) | 21 | 7, 8, 17, 26, 33, 38, 45, 49, 50, 55, 62, 63, 64, 71, 77, 78, 80, 84, 93, 103, 104 |
| BOUNDARY — another module's class | 8 | 1 (M7), 60 (M7), 70 (M7/ESLint), 79 (M1), 86 (M1), 87 (M1), 105 (M7), 106 (M4) |
| EXCLUDED — not a reinvention / tone | 13 | 9, 10, 14, 20, 25, 46, 48, 54, 56, 85, 91, 94, 97 |

(Counting note: rows carrying "+ M1/M7/M9 check" are tallied under their primary verdict; the check
can only move them toward BOUNDARY, never toward shipping without the check. Exact arithmetic:
5 SHIPPED + 25 YES = 31 on the graduation track; 34 MAYBE + 21 NO + 8 BOUNDARY + 13 EXCLUDED = 76
not currently graduating. 31 + 76 = 106.)

**Reading the split honestly:**

- The 26 YES entries are *candidates*, not detectors. Every one still owes a paired
...
- The 34 MAYBEs are deferred for four named reasons: cross-statement correlation (73/74/75),
  dep-gates not yet generalized (6/15/40/58/59/82), an unresolved module boundary (18/47/65/67/
  72/74/102), or unknown FP surface on a loop/heuristic shape (the rest). Each is re-openable by
  resolving its named blocker — they are not soft rejections.
...
3. **The boundary checks** — 18/47/65/67/72/74/102 each name the module to reconcile with before
   any code.
