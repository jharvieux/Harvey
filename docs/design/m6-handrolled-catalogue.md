...
1. **Measure real frequency** — DONE 2026-07-16 (#406 item 1): `pnpm handrolled-frequency`,
   ...
   corpus tier.
2. **Next detector batch** — the YES column, in corpus-measured order (see
   `m6-corpus-frequency.md`: 28, 41, 88, 81, then 29/37/42/52; the 17 measured-zero YES entries
   are unranked), fixtures first. The dep-gate entries need `hasClassMergeDep` generalized to a
   per-library gate.
