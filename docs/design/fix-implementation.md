...
| `src/fix/escalation.ts` | The §5 ladder as pure functions: `planningTriggers()`, `initialTier()`, and `runEscalation()` — max 2 attempts per tier, cheap→standard on verification failure, downgrade with the trigger named, no flagship failure-retries in Phase 1. Feeds `FixResult.tiersUsed`. | §5 | #922 |
...
**Blocker CLEARED, re-measured 2026-07-25 (#1033 sweep).** This paragraph previously said §8 classes 1 (zero-row update returning `error: null`), 2 (unchecked Supabase mutation) and 5 (`void`-prefixed async) had "no detector anywhere in `src/`" and that the fixtures were blocked behind building one. **#1058 landed `harvey-zero-row-update`, `harvey-unchecked-mutation` and `harvey-void-async`** (`src/scan/rules/semgrep/silent-failure.yml`) and `src/fix/calibration-acceptance.test.ts` runs the full §8 gate for all three to GREEN. The same sentence also lived in `docs/design/fix-calibration-acceptance.md` — two documents citing one measurement is not corroboration, and both are corrected here. Re-check with `grep -c "id: harvey" src/scan/rules/semgrep/silent-failure.yml`.

**#1009's remainder after this slice:** the diff-**generating** LLM implementer (a). Clause (c) — detectors-then-fixtures for §8 classes 1/2/5 — is closed by #1058/#1021.
