...
**What this is:** the RAW, UNGRADED output of running Harvey's source/mechanical tier over the ~16
...
**16 targets scanned, 0 dropped. 36,298 files (25,678 `.ts/.tsx`). 39,807 ungraded findings.**
Per-target wall-clock 16–187 s (grida the slowest, at 3,496 `.ts`); no scanner hit the 300 s cap.
These numbers are the whole point of the ungraded warning above: 39,807 is a count of raw emissions,
...
| documenso | prisma | 2822 (2061) | 3022 | 82.6 | 581 | 2 | 2110 | 21 | 258 | 2 | 1 | 47 |
...
Per-module totals (raw emissions, ungraded): **M5 18,985 · M1 7,970 · M4 6,734 · M7 2,225 · M8
1,307 · M6 1,218 · M9 897 · M10 471.** Per-severity: **Low 25,262 · Medium 5,321 · Info 4,658 ·
High 2,946 · Perf 1,529 · Critical 91.** The 91 "Critical" rows are the sharpest illustration of the
...
Six curated corpus targets never stress these; a 16-repo volume sweep does. What it found:

- **CRASH — `mutation-scan` on 3/16 (`liam`, `dub`, `cal-diy`).** Same root cause each time:
...
- **SILENT-EMPTY — none that are real.** `mutation-scan` reads `empty` on 6/16, but that is the
  *correct* output: a test suite WAS present, so the suite-absent finding correctly did not fire.
  `quick-scan`, `detect-static`, `quality-scan` and `pii-classify` emitted on all 16 — no module
...
- **OVER-FIRE candidates (for #900, not graded here).** M5 dominates every large repo (`cal-diy`
  4,377; `priceai`/`grida`/`documenso`/`amplication` each >1,900) and `hexclave`'s M8 test-intent
