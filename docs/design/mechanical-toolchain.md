...
- **FP control:** per-rule `metadata: confidence: HIGH/MEDIUM/LOW` + `--severity ERROR` to gate the count; `// nosemgrep: rule-id`; `.semgrepignore` for `.next`/generated/tests. Start from high-confidence, medium/high-impact rules only (Trail of Bits).
...
|---|---|---|---|---|
| **Semgrep OSS** | Mechanical-security core (SAST + custom taxonomy) | `p/typescript`, `p/react`, `p/nextjs`, `p/owasp-top-ten`; custom: service-role-in-client, `NEXT_PUBLIC` leak, missing-auth-guard | `confidence: HIGH` + `--severity ERROR`; exclude `.audit.*`; `nosemgrep`; `.semgrepignore` | HIGH-conf ERROR rules: **YES**; MEDIUM/heuristic: triage |
...
| **gitleaks** | Fast pre-commit + custom Supabase secret rules | custom `supabase-service-role-jwt` w/ `--max-decode-depth 2`; anon-key allowlist; `.env.example` allowlist | rule allowlists; decode `role` claim | custom service-role hit: **YES**; generic: triage |
